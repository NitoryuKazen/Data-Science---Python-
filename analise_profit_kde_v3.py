...
def make_image(sample,bcg_list,idx_z):
	...
	for galaxia in bcg_list:
		if os.path.isfile(f'{sample}_stats_observation/p_value/{idx_z}/bcg_depot/{galaxia}.png'):
			pass
		else:
			print(galaxia,idx_z)
			...
			plt.savefig(f'{sample}_stats_observation/p_value/{idx_z}/bcg_depot/{galaxia}.png')
			plt.close()
	return
...
def gmm_bootstrap(sample,delta_bic,savedir,n,n_bins):
	...
	path=f'{sample}_stats_observation/redshift_gmm/{n_bins}_bins/'
...
# def svc_calc(vecs_x,vecs_y,name,outfile):
...
'''
#SERSIC
'XCEN' 'YCEN' 'MAG' 'RE' 'NSER' 'ANG' 'AXRAT' 'BOX' 'SKY' 'MAX_LLH' 'BIC' 'CL_LLH' 

#SERSIC DUPLO
'XCEN_1' 'YCEN_1' 'MAG_1' 'RE_1' 'NSER_1' 'ANG_1' 'AXRAT_1' 'BOX_1' 'MAG_2'
'RE_2' 'NSER_2' 'ANG_2' 'AXRAT_2' 'BOX_2' 'SKY' 'MAX_LLH' 'BIC' 'CL_LLH'

#MISCELANEOUS
'cluster' 'rff' 'ass' 'best_model' 'delta_s' 'delta_se'
'''		
sample='WHL'

# if sample == 'WHL':
# 	DEFAULT_S = 12
# 	_original_scatter = plt.scatter
# 	def scatter_with_default_s(*args, **kwargs):
# 		kwargs.setdefault('s', DEFAULT_S)
# 		return _original_scatter(*args, **kwargs)

# 	plt.scatter = scatter_with_default_s

#PATHS
path_dad=f'{sample}_stats_observation/kormendy_rel',f'{sample}_stats_observation/scatter_re_n'

files='bojo_env','comp_12'

paths_sons=[f'{path_dad[0]}/{files[i]}' for i in range(2)],[f'{path_dad[1]}/{files[i]}' for i in range(2)]

[path_bojo_env_mue,path_12_mue],[path_bojo_env_mue_med,path_12_mue_med]=paths_kormendy=[f'{paths_sons[0][i]}/mue' for i in range(2)],[f'{paths_sons[0][i]}/mue_med' for i in range(2)]

[path_bojo_env_geral_vlog,path_12_geral_vlog],[path_bojo_env_geral_linear,path_12_geral_linear]=paths_scatter_re_n_geral=[f'{paths_sons[1][i]}/geral/vlog' for i in range(2)],[f'{paths_sons[1][i]}/geral/linear' for i in range(2)]

[path_bojo_env_subplot_vlog,path_12_subplot_vlog],[path_bojo_env_subplot_linear,path_12_subplot_linear]=paths_scatter_re_n_subplots=[f'{paths_sons[1][i]}/subplots/vlog' for i in range(2)],[f'{paths_sons[1][i]}/subplots/linear' for i in range(2)]

for path in paths_kormendy:
	for i in range(2):
			os.makedirs(path[i],exist_ok=True)
for path in paths_scatter_re_n_geral:
	for i in range(2):
		os.makedirs(path[i],exist_ok=True)
for path in paths_scatter_re_n_subplots:
	for i in range(2):
		os.makedirs(path[i],exist_ok=True)
...
#DATA FILES
data_chi2_temp=np.loadtxt(f'{sample}_files/{sample}_profit_observation_SE_sky_chi2.dat',dtype=str).T
data_psf_temp=np.loadtxt(f'{sample}_files/{sample}_psf_data.dat',dtype=str).T
data_z_temp=np.loadtxt(f'{sample}_files/{sample}_clean_redshift.dat',dtype=str).T
data_eta_temp=np.loadtxt(f'{sample}_files/ass_{sample}.dat').T
...
data_corr_temp=np.loadtxt(f'{sample}_files/{sample}_corr_bt.dat',dtype=str).T
...
rff_ss=np.loadtxt(f'{sample}_files/{sample}_rff_duplo.dat',dtype=float,usecols=[1]).T
...
# bic_sersic_obs_psf=k1*np.log(n_data) - 2*(llh_sersic_obs)
# bic_sersic_duplo_obs_psf=k2*np.log(n_data) - 2*(llh_sersic_duplo_obs)

# bic_sersic_sim_psf=k1*np.log(n_data) - 2*(llh_sersic_sim)
# bic_sersic_duplo_sim_psf=k2*np.log(n_data) - 2*(llh_sersic_duplo_sim)

# delta_bic_obs_psf=bic_sersic_duplo_obs_psf - bic_sersic_obs_psf
# delta_bic_sim_psf=bic_sersic_duplo_sim_psf - bic_sersic_sim_psf
...
# mag_2_fake=np.loadtxt(f'{sample}_files/mag_artificial_WHL.dat',dtype=float,usecols=[1]).T[lim_finite & cut_lim]
# mag_c1_fake,mag_c2_fake=np.loadtxt(f'{sample}_files/mag_artificial_c1_c2_WHL.dat',dtype=float,usecols=[1,2])[lim_finite & cut_lim].T
...
# bt_vec_corr_env_fake=bt_ratio_corr(mag1,mag_2_fake,corr_1,corr_2)

# bt_vec_corr_fake=bt_ratio_corr(mag_c1_fake,mag_c2_fake,corr_1,corr_2)
...
# slope_gr=data_photutils['slope_gr_fix'].astype(float)[lim_finite & cut_lim][lim_photutils]
...
slope_a3_err=np.ones_like(grad_e_err)#1./np.power(slope_a3_err,2)
slope_a4_err=np.ones_like(grad_e_err)#1./np.power(slope_a4_err,2)
slope_b3_err=np.ones_like(grad_e_err)#1./np.power(slope_b3_err,2)
slope_b4_err=np.ones_like(grad_e_err)#1./np.power(slope_b4_err,2)
slope_disk_err=np.ones_like(grad_e_err)#1./np.power(slope_disk_err,2)
slope_gr_err=np.ones_like(grad_e_err)#1./np.power(slope_gr_err,2)
grad_e_err=np.ones_like(grad_e_err)#1./np.power(grad_e_err,2)
grad_pa_err=np.ones_like(grad_pa_err)#1./np.power(grad_pa_err,2)
...
r'''
for n in num:
	# print(np.average(redshift[idx_faixas==n]),len(redshift[idx_faixas==n]))
	os.makedirs(f'{sample}_stats_observation/p_value/{file_names[n-1]}',exist_ok=True)
	fig,axs=plt.subplots(1,1,tight_layout=True,figsize=(10,8))
	axs.set_title(title_vec[n-1])
	axs.scatter(redshift[idx_faixas==n],p_value[idx_faixas==n],color='white',edgecolor='black',alpha=0.9)
	...
	axs.set_ylim(0,1)
	# plt.show()
	plt.savefig(f'{sample}_stats_observation/p_value/{file_names[n-1]}/p_value_z.png')
	plt.close(fig)

	lim1=rff_ratio[idx_faixas==n]>0.9
	delta_bic_obs_09=delta_bic_obs[idx_faixas==n][lim1]
	delta_bic_obs_res=delta_bic_obs[idx_faixas==n][~lim1]
	# quad1=(p_value>0.32) & (delta_bic_obs<np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.)
	# quad2=(p_value>0.32) & (delta_bic_obs>np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.)
	# quad3=(p_value<0.32) & (delta_bic_obs>np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.)
	# quad4=(p_value<0.32) & (delta_bic_obs<np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.)


	# region_vec=[quad1,quad2,quad3,quad4]
	# morf_vec=[elip_cut,cd_cut,(ecd_cut) | (cde_cut)]
	# morf_str=['E','cD','E/cD']
	# for j,item in enumerate(region_vec):
	# 	print(j+1)
	# 	tot=len(cluster)
	# 	tot_quad=len(cluster[item])
	# 	tot_per=(tot_quad/tot)*100.
	# 	print('total',len(cluster[item]),tot_per) 
	# 	for i,tipo in enumerate(morf_vec):
	# 		tot_morf=len(cluster[tipo])
	# 		tot_quad_morf=len(cluster[item & tipo])
	# 		tot_per_morf=(tot_quad_morf/tot_morf)*100.
	# 		print(morf_str[i],tot_quad_morf,tot_per_morf)
	# 	print('########')
	# print(cluster[(p_value<0.32) & (p_value>0.0456) & (delta_bic_obs>np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.)])
	fig,axs=plt.subplots(1,1,tight_layout=True,figsize=(10,8))
	axs.set_title(title_vec[n-1])
	...
	axs.axhline(0.32,label=r'$1 \\sigma$',)
	axs.axvline(np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.,color='black',label=f'{str(np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.)}')
	axs.legend()
	...
	cbar.set_label('B/T')
	# plt.show()
	plt.savefig(f'{sample}_stats_observation/p_value/{file_names[n-1]}/p_value_delta_bic_bt.png')
	plt.close(fig)

	if sample=='L07':
		fig,axs=plt.subplots(1,1,tight_layout=True,figsize=(10,8))
		axs.set_title(title_vec[n-1])
		axs.scatter(delta_bic_obs[idx_faixas==n][elip_cut[idx_faixas==n]],p_value[idx_faixas==n][elip_cut[idx_faixas==n]],color='green',edgecolor='black',alpha=0.9,label='E')
		axs.scatter(delta_bic_obs[idx_faixas==n][cd_cut[idx_faixas==n]],p_value[idx_faixas==n][cd_cut[idx_faixas==n]],color='blue',edgecolor='black',alpha=0.9,label='cD')
		axs.scatter(delta_bic_obs[idx_faixas==n][ecd_cut[idx_faixas==n] | cde_cut[idx_faixas==n]],p_value[idx_faixas==n][ecd_cut[idx_faixas==n] | cde_cut[idx_faixas==n]],color='red',edgecolor='black',alpha=0.9,label='E/cD')
		axs.axhline(0.32,label=r'$1 \\sigma$')
		axs.axhline(0.0456,label=r'$2 \\sigma$')
		axs.axvline(np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.,color='black',label=f'{str(np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.)}')
		axs.legend()
		axs.set_xlabel(r'$\\Delta BIC$')
		axs.set_ylabel(r'$P_{value}$')
		axs.set_xlim(2000,-8000)
		axs.set_ylim(0,1)
		plt.savefig(f'{sample}_stats_observation/p_value/{file_names[n-1]}/p_value_delta_bic_morf.png')
		plt.close(fig)

	fig,axs=plt.subplots(1,1,tight_layout=True,figsize=(10,8))
	axs.set_title(title_vec[n-1])
	axs.scatter(delta_bic_obs[idx_faixas==n],p_value[idx_faixas==n],color='white',edgecolor='black',alpha=0.9)
	axs.axhline(0.32,label=r'$1 \\sigma$')
	axs.axvline(np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.,color='black',label=f'{str(np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.)}')
	axs.legend()
	axs.set_xlabel(r'$\\Delta BIC$')
	...
	axs.set_xlim(2000,-8000)
	axs.set_ylim(0,1)
	plt.savefig(f'{sample}_stats_observation/p_value/{file_names[n-1]}/p_value_delta_bic.png')
	plt.close(fig)


	fig,axs=plt.subplots(1,1,tight_layout=True,figsize=(10,8))
	axs.set_title(title_vec[n-1])
	sc=axs.scatter(delta_bic_obs[idx_faixas==n],p_value[idx_faixas==n],c=n2[idx_faixas==n],edgecolor='black',vmin=0,vmax=7,cmap=cmap)
	axs.axhline(0.32,label=r'$1 \\sigma$')
	axs.axvline(np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.,color='black',label=f'{str(np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.)}')
	axs.legend()
	axs.set_xlabel(r'$\\Delta BIC$')
	...
	plt.savefig(f'{sample}_stats_observation/p_value/{file_names[n-1]}/p_value_delta_bic_n2.png')
	plt.close(fig)

	fig,axs=plt.subplots(1,1,tight_layout=True,figsize=(10,8))
	axs.set_title(title_vec[n-1])
	sc=axs.scatter(delta_bic_obs[idx_faixas==n],p_value[idx_faixas==n],c=re_ratio_12[idx_faixas==n],edgecolor='black',vmin=0,vmax=2,cmap=cmap)
	axs.axhline(0.32,label=r'$1 \\sigma$')
	axs.axvline(np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.,color='black',label=f'{str(np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.)}')
	axs.legend()
	axs.set_xlabel(r'$\\Delta BIC$')
	...
	axs.set_ylim(0,1)
	cbar=plt.colorbar(sc, ax=axs)
	cbar.set_label(r'$R_e1/R_e2$')
	plt.savefig(f'{sample}_stats_observation/p_value/{file_names[n-1]}/p_value_delta_bic_.png')
	plt.close(fig)

	fig,axs=plt.subplots(1,1,tight_layout=True,figsize=(10,8))
	axs.set_title(title_vec[n-1])
	sc=axs.scatter(delta_bic_obs[idx_faixas==n],p_value[idx_faixas==n],c=rff_ratio[idx_faixas==n],edgecolor='black',vmin=0,vmax=1,cmap=cmap)
	axs.axhline(0.32,label=r'$1 \\sigma$')
	axs.axvline(np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.,color='black',label=f'{str(np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.)}')
	axs.legend()
	axs.set_xlabel(r'$\\Delta BIC$')
	...
	axs.set_ylim(0,1)
	cbar=plt.colorbar(sc, ax=axs)
	cbar.set_label(r'$RFF_s/RFF_{ss}$')
	# plt.show()
	plt.savefig(f'{sample}_stats_observation/p_value/{file_names[n-1]}/p_value_delta_bic_rff_ratio.png')
	plt.close(fig)

	kde_sample=kde(delta_bic_obs[idx_faixas==n][np.isfinite(delta_bic_obs[idx_faixas==n])])
	bw_factor = kde_sample.factor
	kde_sub_samples=[]
	x=np.linspace(min(delta_bic_obs[idx_faixas==n]),max(delta_bic_obs[idx_faixas==n]),12000)

	y1=kde(delta_bic_obs_09,bw_method=bw_factor/2.)
	y2=kde(delta_bic_obs_res,bw_method=bw_factor/2.)
	fig,axs=plt.subplots(1,1)#2,int(len(sub_samples)/2),figsize=(15,10),tight_layout=True)
	axs.set_title(title_vec[n-1])
	axs.plot(x,y1(x),label='rff_ratio>09')
	axs.plot(x,y2(x),label='resto')
	axs.axvline(np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.,label=f'{str(np.median(delta_bic_obs_09)-mad(delta_bic_obs_09)*1.4826*3.)}')
	axs.set_xlabel(r'$\\Delta BIC$')
	axs.set_xlim(-2000,3000)
	axs.legend()
	# plt.show()
	plt.savefig(f'{sample}_stats_observation/p_value/{file_names[n-1]}/delta_bic_kde_.png')
		plt.close()
'''
#################################
#CORTES ATUAIS E RELATIVOS A RELAÇÃO DE KORMENDY E P VALUE
...
title_vec=['0.0733', '0.0999', '0.113', '0.123', '0.133', '0.141',
		 '0.149', '0.156', '0.162', '0.168']

file_names=[str(j) for j in range(len(title_vec))]
vec_cuts=[]
z_cuts=[]
...
#,cov=scp.curve_fit(log_model,z_cuts,vec_cuts,p0=[-13.9,11.55])
pov=[531.9682942,931.78910737]
x1,x2=min(redshift),max(redshift)
xz=np.linspace(x1,x2,len(delta_bic_obs))

cd_lim=delta_bic_obs < log_model(xz,*pov)
elip_lim= ~cd_lim
...
low_lim_elip=elip_lim & n1_low
low_lim_cd=cd_lim & n1_low

high_lim_elip=elip_lim & n1_high
high_lim_cd=cd_lim & n1_high
higher_lim_cd=cd_lim & n1_higher
...
x1ss,x2ss=(-0.28322921653227484,2.716867381942037)#x1ss,x2ss)
...
label_bojo='Comp 1 Class.'
label_env='Comp 2 Class.'
label_bojo_el='Comp 1 Nuclear'
label_env_el='Comp 2 Nuclear'

linha_interna_label='Linha comp 1'
linha_externa_label='Linha comp 2'

linha_interna_el_label='Linha comp 1 Nuclear'
linha_externa_el_label='Linha comp 2 Nuclear'
####
...
axs.legend()
# plt.show()
plt.savefig(f'{sample}_stats_observation/p_value/rel_kormendy_cd_e.png')
plt.close()
...
axs.legend()
# plt.show()
plt.savefig(f'{sample}_stats_observation/p_value/pre_kormendy_rel_comp.png')
plt.close()
...
lim_cd_small=cd_lim & (re_1_kpc<0.673)
lim_cd_big=cd_lim & (re_1_kpc>0.673)

# cluster_n1=cluster
# ra_n1=ra[lim_cd_small]
# dec_n1=dec[lim_cd_small]
# # redshift_n1=redshift[]
# # n1_cd=n1[cd_lim]
# data_n1=np.asarray([cluster,ra,dec]).T#,redshift_n1,n1_cd]).T
# np.savetxt(f'desi_indiv_{sample}.dat',data_n1,fmt='%s',newline='\n')
# print(cluster[lim_cd_small],ra[lim_cd_small],dec[lim_cd_small])
...
plt.figure()
plt.scatter(re_ratio_12[lim_cd_big & lim_photutils][n_1d_s[cd_lim_photutils_big]<15.],n_1d_2[cd_lim_photutils_big][n_1d_s[cd_lim_photutils_big]<15.])
plt.scatter(re_ratio_12[lim_cd_small & lim_photutils][n_1d_s[cd_lim_photutils_small]<15.],n_1d_2[cd_lim_photutils_small][n_1d_s[cd_lim_photutils_small]<15.])
# plt.scatter(n_s[lim_photutils][n_1d_s<15.],n_1d_s[n_1d_s<15.])
plt.show()
plt.close()
...
vec_name=['E','E(EL)','True cD']
c_vec=['green','blue','red']
for i,n in enumerate(num):
	z_sub_sample_only=idx_faixas==n
	cutoff=vec_cuts[i]
	# print(cutoff,z_cuts[vec_cuts.index(cutoff)])
	# os.makedirs(f'{sample}_stats_observation/p_value/{file_names[n-1]}',exist_ok=True)
	
	cd_z_lim=delta_bic_obs[z_sub_sample_only]>cutoff
	elip_z_lim=~cd_z_lim

	lim_small_center_z=(cd_z_lim) & (re_1_kpc[z_sub_sample_only]<0.673)
	lim_big_center_z=(cd_z_lim) & (re_1_kpc[z_sub_sample_only]>0.673)
	sub_sample_n=n_s[z_sub_sample_only]
	n_sz[0].append(np.average(sub_sample_n[elip_z_lim]))
	n_sz[1].append(np.average(sub_sample_n[lim_small_center_z]))
	n_sz[2].append(np.average(sub_sample_n[lim_big_center_z]))

	err_n_sz[0].append(np.std(sub_sample_n[elip_z_lim])/len(elip_z_lim))
	err_n_sz[1].append(np.std(sub_sample_n[lim_small_center_z])/len(sub_sample_n[lim_small_center_z]))
	err_n_sz[2].append(np.std(sub_sample_n[lim_big_center_z])/len(sub_sample_n[lim_big_center_z]))

plt.figure()
for i,vec in enumerate(n_sz):
	plt.errorbar(z_cuts,vec,yerr=err_n_sz[i],marker='o',color=c_vec[i],label=vec_name[i])
plt.legend()
# plt.show()
plt.close()
#LINHA 1352 ATÉ 2660 SÃO OS TESTES KDE
##
#DEFINIÇÃO DOS PARÂMETROS/LIMITES/DISTRIBUIÇÕES DE INTERESSE E FATORES PARA OS PLOTS KDE
####
#PRÉ DEFINIÇÃO DOS PARAMETROS COM OS LIMITES AMOSTRAIS E PARAMÉTRICOS (PARA AQUELES QUE NÃO POSSUEM)
...
##
os.makedirs(f'{sample}_stats_observation/test_ks/indice_sersic',exist_ok=True)
#MODELO SIMPLES
plt.figure()
...
plt.savefig(f'{sample}_stats_observation/test_ks/indice_sersic/n_simples_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/indice_sersic/n_simples_dvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/indice_sersic/n_interno_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/indice_sersic/n_interno_dvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/indice_sersic/n_externo_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/indice_sersic/n_externo_dvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/indice_sersic/n_simples_kde.png')
# plt.show()
plt.close()
...
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
# plt.show()
plt.savefig(f'{sample}_stats_observation/test_ks/indice_sersic/n_interno_kde.png')
plt.close()
...
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
# plt.show()
plt.savefig(f'{sample}_stats_observation/test_ks/indice_sersic/n_externo_kde.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/indice_sersic/n_externo_EL_E_kde.png')
plt.close()
...
#IMAGENS RAZÃO AXIAL
os.makedirs(f'{sample}_stats_observation/test_ks/ax_ratio',exist_ok=True)
...
plt.savefig(f'{sample}_stats_observation/test_ks/ax_ratio/axrat_simples_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/ax_ratio/axrat_simples_dvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/ax_ratio/axrat_interno_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/ax_ratio/axrat_interno_dvalue.png')
plt.close()
##
plt.figure()
...
plt.savefig(f'{sample}_stats_observation/test_ks/ax_ratio/axrat_externo_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/ax_ratio/axrat_externo_dvalue.png')
plt.close()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/ax_ratio/axrat_simples_kde.png')
plt.close()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/ax_ratio/axrat_interno_kde.png')
plt.close()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/ax_ratio/axrat_externo_kde.png')
plt.close()
...
info_labels = (f'K-S(E,E(EL)) ={ks_e_el:.3e}')
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/ax_ratio/axrat_externo_EL_E_kde.png')
plt.close()
...
os.makedirs(f'{sample}_stats_observation/test_ks/box',exist_ok=True)
...
plt.savefig(f'{sample}_stats_observation/test_ks/box/box_simples_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/box/box_simples_dvalue.png')
plt.close()
##
plt.figure()
...
plt.savefig(f'{sample}_stats_observation/test_ks/box/box_interno_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/box/box_interno_dvalue.png')
plt.close()
##
plt.figure()
...
plt.savefig(f'{sample}_stats_observation/test_ks/box/box_externo_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/box/box_externo_dvalue.png')
plt.close()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/box/box_simples_kde.png')
plt.close()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/box/box_interno_kde.png')
plt.close()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/box/box_externo_kde.png')
plt.close()
...
info_labels = (f'K-S(E,E(EL)) ={ks_e_el:.3e}')
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/box/box_externo_EL_E_kde.png')
plt.close()
##############################################################
#RFFs/RFFss
##SÉRSIC -- cD,E(EL),cD(std),E
...
plt.savefig(f'{sample}_stats_observation/test_ks/a3/slope_a3_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/a3/slope_a3_dvalue.png')
plt.close()
...
for i,dist in enumerate(vec_kde_a3):
	...
	axs.axvline(vec_med_a3[i],color=cores[i],ls='--',label=fr'$\mu = {vec_med_a3[i]:.3f}$')
axs.legend()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/a3/a3_simples_kde.png')
plt.close()
...
for i,dist in enumerate(vec_kde_slope_a3):
	...
	axs.axvline(vec_med_slope_a3[i],color=cores[i],ls='--',label=fr'$\mu = {vec_med_slope_a3[i]:.3f}$')
axs.legend()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/a3/slope_a3_simples_kde.png')
plt.close()
...
slope_a4_linspace=np.linspace(-0.02,0.02,3000)
...
plt.savefig(f'{sample}_stats_observation/test_ks/a4/slope_a4_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/a4/slope_a4_dvalue.png')
plt.close()
...
for i,dist in enumerate(vec_kde_a4):
	...
	axs.axvline(vec_med_a4[i],color=cores[i],ls='--',label=fr'$\mu = {vec_med_a4[i]:.3f}$')
axs.legend()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/a4/a4_simples_kde.png')
plt.close()
...
for i,dist in enumerate(vec_kde_slope_a4):
	...
	axs.axvline(vec_med_slope_a4[i],color=cores[i],ls='--',label=fr'$\mu = {vec_med_slope_a4[i]:.3f}$')
axs.legend()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/a4/slope_a4_simples_kde.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/b3/slope_b3_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/b3/slope_b3_dvalue.png')
plt.close()
...
for i,dist in enumerate(vec_kde_b3):
	...
	axs.axvline(vec_med_b3[i],color=cores[i],ls='--',label=fr'$\mu = {vec_med_b3[i]:.3f}$')
axs.legend()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/b3/b3_simples_kde.png')
plt.close()
...
for i,dist in enumerate(vec_kde_slope_b3):
	...
	axs.axvline(vec_med_slope_b3[i],color=cores[i],ls='--',label=fr'$\mu = {vec_med_slope_b3[i]:.3f}$')
axs.legend()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/b3/slope_b3_simples_kde.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/b4/slope_b4_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/b4/slope_b4_dvalue.png')
plt.close()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/b4/b4_simples_kde.png')
plt.close()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/b4/slope_b4_simples_kde.png')
plt.close()
...
os.makedirs(f'{sample}_stats_observation/test_ks/test_gr',exist_ok=True)
...
plt.savefig(f'{sample}_stats_observation/test_ks/test_gr/slope_gr_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/test_gr/slope_gr_dvalue.png')
plt.close()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/test_gr/slope_gr_simples_kde.png')
plt.close()
...
os.makedirs(f'{sample}_stats_observation/test_ks/grad_e',exist_ok=True)
...
plt.savefig(f'{sample}_stats_observation/test_ks/grad_e/grad_e_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/grad_e/grad_e_dvalue.png')
plt.close()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/grad_e/grad_e_kde.png')
plt.close()
...
os.makedirs(f'{sample}_stats_observation/test_ks/grad_pa',exist_ok=True)

plt.figure()
plt.title('Gradiente de PA - p_value')
sns.heatmap(ks_grad_e[0],xticklabels=names_simples,yticklabels=names_simples, annot=True, cmap='Reds_r')
plt.savefig(f'{sample}_stats_observation/test_ks/grad_pa/grad_pa_pvalue.png')
plt.close()

plt.figure()
plt.title('Gradiente de PA - D_value')
sns.heatmap(ks_grad_e[1],xticklabels=names_simples,yticklabels=names_simples, annot=True, cmap='Reds')
plt.savefig(f'{sample}_stats_observation/test_ks/grad_pa/grad_pa_dvalue.png')
plt.close()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/grad_pa/grad_pa_kde.png')
plt.close()
...
os.makedirs(f'{sample}_stats_observation/test_ks/magabs',exist_ok=True)
...
plt.savefig(f'{sample}_stats_observation/test_ks/magabs/magabs_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/magabs/magabs_dvalue.png')
plt.close()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/magabs/magabs_kde.png')
plt.close()
...
os.makedirs(f'{sample}_stats_observation/test_ks/starmass',exist_ok=True)
...
plt.savefig(f'{sample}_stats_observation/test_ks/starmass/starmass_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/starmass/starmass_dvalue.png')
plt.close()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/starmass/starmass_kde.png')
plt.close()
...
os.makedirs(f'{sample}_stats_observation/test_ks/age',exist_ok=True)
...
plt.savefig(f'{sample}_stats_observation/test_ks/age/age_pvalue.png')
plt.close()
...
plt.savefig(f'{sample}_stats_observation/test_ks/age/age_dvalue.png')
plt.close()
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/age/age_kde.png')
plt.close()
...
# plt.show()
plt.savefig(f'{sample}_stats_observation/test_ks/starmass_m200.png')
plt.close()
...
# plt.show()
plt.savefig(f'{sample}_stats_observation/test_ks/starmass_age.png')
plt.close()
# 
# 
...
#
mass_c1_e=np.log10(np.multiply(bt_e[lim_casjobs[elip_lim]],np.power(10,starmass_e)))
mass_c2_e=np.log10(np.multiply((1-bt_e[lim_casjobs[elip_lim]]),np.power(10,starmass_e)))

mass_c1_cd_small=np.log10(np.multiply(bt_cd_small[lim_casjobs[lim_cd_small]],np.power(10,starmass_cd_small)))
mass_c2_cd_small=np.log10(np.multiply((1-bt_cd_small[lim_casjobs[lim_cd_small]]),np.power(10,starmass_cd_small)))

mass_c1_cd_big=np.log10(np.multiply(bt_cd_big[lim_casjobs[lim_cd_big]],np.power(10,starmass_cd_big)))
mass_c2_cd_big=np.log10(np.multiply((1-bt_cd_big[lim_casjobs[lim_cd_big]]),np.power(10,starmass_cd_big)))
#
...
fig.legend()
plt.savefig(f'{sample}_stats_observation/test_ks/m200_mass_c1.png')
plt.close()
...
fig.legend()
plt.savefig(f'{sample}_stats_observation/test_ks/m200_mass_elip_elc2.png')
plt.close()
...
fig.legend()
plt.savefig(f'{sample}_stats_observation/test_ks/m200_mass_c2.png')
plt.close()
...
fig.legend()
plt.savefig(f'{sample}_stats_observation/test_ks/m200_mass_comps.png')
plt.close()

###########################################################
#LINHA DO H ALPHA
h_line_cd,h_line_cd_small,h_line_cd_big,h_line_e=h_line[cd_lim[lim_halpha]],h_line[lim_cd_small[lim_halpha]],h_line[lim_cd_big[lim_halpha]],h_line[elip_lim[lim_halpha]]
...
os.makedirs(f'{sample}_stats_observation/test_ks/h_line',exist_ok=True)
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/h_line/hline_kde.png')
# plt.show()
plt.close()
...
os.makedirs(f'{sample}_stats_observation/test_ks/bt',exist_ok=True)
...
# fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
# fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/bt/bt_kde.png')
# plt.show()
plt.close()
...
# fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
# fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/bt/bt_corr_kde.png')
# plt.show()
plt.close()

###########################################################
#RAZÃO DE RAIOS
re_ratio_cd,re_ratio_cd_small,re_ratio_cd_big,re_ratio_e=re_ratio_12[cd_lim],re_ratio_12[lim_cd_small],re_ratio_12[lim_cd_big],re_ratio_12[elip_lim]
#
kde_re_ratio=kde(re_ratio_12)
re_ratio_factor = kde_re_ratio.factor
re_ratio_linspace=np.linspace(min(re_ratio_12),max(re_ratio_12),3000)
re_ratio_kde_cd,re_ratio_kde_cd_small,re_ratio_kde_cd_big,re_ratio_kde_e=kde(re_ratio_cd,bw_method=re_ratio_factor),kde(re_ratio_cd_small,bw_method=re_ratio_factor),kde(re_ratio_cd_big,bw_method=re_ratio_factor),kde(re_ratio_e,bw_method=re_ratio_factor)
##########################################################
#RAZÃO DE INDICES DE SÉRSIC
n_ratio_cd,n_ratio_cd_small,n_ratio_cd_big,n_ratio_e=n_ratio_12[cd_lim],n_ratio_12[lim_cd_small],n_ratio_12[lim_cd_big],n_ratio_12[elip_lim]
#
kde_n_ratio=kde(n_ratio_12)
n_ratio_factor = kde_n_ratio.factor
n_ratio_linspace=np.linspace(min(n_ratio_12),max(n_ratio_12),3000)
##
n_ratio_kde_cd,n_ratio_kde_cd_small,n_ratio_kde_cd_big,n_ratio_kde_e=kde(n_ratio_cd,bw_method=n_ratio_factor),kde(n_ratio_cd_small,bw_method=n_ratio_factor),kde(n_ratio_cd_big,bw_method=n_ratio_factor),kde(n_ratio_e,bw_method=n_ratio_factor)
##########################################################
#RAZÃO DE RAZÃO AXIAL 1 E 2 
axrat_ratio_cd,axrat_ratio_cd_small,axrat_ratio_cd_big,axrat_ratio_e=axrat_ratio_12[cd_lim],axrat_ratio_12[lim_cd_small],axrat_ratio_12[lim_cd_big],axrat_ratio_12[elip_lim]
#
kde_axrat_ratio=kde(axrat_ratio_12)
axrat_ratio_factor = kde_axrat_ratio.factor
axrat_ratio_linspace=np.linspace(min(axrat_ratio_12),max(axrat_ratio_12),3000)
##
axrat_ratio_kde_cd,axrat_ratio_kde_cd_small,axrat_ratio_kde_cd_big,axrat_ratio_kde_e=kde(axrat_ratio_cd,bw_method=axrat_ratio_factor),kde(axrat_ratio_cd_small,bw_method=axrat_ratio_factor),kde(axrat_ratio_cd_big,bw_method=axrat_ratio_factor),kde(axrat_ratio_e,bw_method=axrat_ratio_factor)
#############################################
#CONCENTRÃÇO
...
os.makedirs(f'{sample}_stats_observation/test_ks/conc',exist_ok=True)
...
fig.text(0.7, 0.95, info_labels, fontsize=11, va='center', ha='left')
fig.text(0.1, 0.95, info_labels_2c, fontsize=11, va='center', ha='left')
plt.savefig(f'{sample}_stats_observation/test_ks/conc/conc_kde.png')
plt.close()
...
# par_vec=[e_s,e1,e2,box_s,bt_vec_12,bt_vec_corr,chi2_ratio,n1,n_s,axrat_ratio_12,n_ratio_12,rff_ratio,np.log10(re_ratio_12),n2,box1,box2,ass,np.log10(rff_s),eta,slope_a3,slope_a4,slope_b3,slope_b4,slope_gr,grad_e,grad_pa]#]
...
# 	par_temp=[par_vec[ind1][cd_lim & np.isfinite(np.log10(rff_s))],par_vec[ind2][cd_lim & np.isfinite(np.log10(rff_s))]]

# 	vec_x_el_cd=np.column_stack(par_temp)
# 	split_cd_el=svc_calc(vec_x_el_cd,lim_cd_small[cd_lim & np.isfinite(np.log10(rff_s))].astype(int).T,dupla,f'{sample}_stats_observation/svc_score_el_cd_dupla_{sample}.dat')
...
# 	par_temp=[par_vec[ind1][cd_lim_casjobs & np.isfinite(np.log10(rff_s[lim_casjobs]))],par_vec[ind2][cd_lim_casjobs & np.isfinite(np.log10(rff_s[lim_casjobs]))],par_vec[ind3][cd_lim_casjobs & np.isfinite(np.log10(rff_s[lim_casjobs]))]]

# 	vec_x_el_cd=np.column_stack(par_temp)
# 	split_cd_el=svc_calc(vec_x_el_cd,lim_cd_small[cd_lim & np.isfinite(np.log10(rff_s)) & lim_casjobs].astype(int).T,trio,f'{sample}_stats_observation/svc_score_el_cd_{sample}_casjobs.dat')
'''
###############################################
...
dlpov=[1.54622724e-02,4.61969720e-03,1.23642039e+02,-3.93661732e+00,-4.81476825e-01,7.57926648e+01]# p0=[np.random.normal(0.0156,0.002),np.random.normal(0.004,0.01),np.random.normal(144,10),np.random.normal(3.9,1),np.random.normal(0.5,0.3),np.random.normal(88,20)]
ynorm=[]
idx_split_rff = np.argwhere(np.diff(np.sign(diff(x0,dlpov)))).flatten()

bins = np.arange(0,0.1,0.0005)
fig0=plt.figure(figsize=(9,7))
y,x,_ = plt.hist(rff_s,bins=bins,histtype='step',color='white',edgecolor='black')
x = (x[1:]+x[:-1])/2
for i in range(len(y)):
	if y[i] == 0.0:
		pass
	else:
		ynorm.append(np.power(y[i]-dlognorm(x,*dlpov)[i],2)/y[i])
...
plt.ylabel('Objetos')
# plt.show()
# plt.savefig(f'{sample}_stats_observation/plano_rff_eta/histogram_rff_lines.png')
plt.close(fig0)
...
plt.plot(x0,dlognorm(x0,*dlpov),linewidth=2, color='b',label='G+log')
plt.plot([],[],color='b',linewidth=2,label = r'$\chi^{2}$ '+str(format(abs(chi2_norm), '.4')))
plt.plot(x0,gauss(x0,*dlpov[:3]),linewidth=1.5,ls=':',color='black',label='G'+'\n'+'A '+ str(format(abs(dlpov[:3][2]),'.3f'))+'\n'+r'$\mu$ '+str(format(abs(dlpov[:3][0]),'.3f'))+ '\n'+ r'$\sigma$ '+str(format(abs(dlpov[:3][1]),'.3f')))
plt.plot(x0,lognormal(x0,*dlpov[3:6]),linewidth=1.5,ls='--',color='g',label='log'+'\n'+'A '+ str(format(abs(dlpov[3:6][2]),'.3f'))+'\n'+r'$\mu$ '+str(format(abs(dlpov[3:6][0]),'.3f'))+ '\n'+ r'$\sigma$ '+str(format(abs(dlpov[3:6][1]),'.3f')))
plt.axvline(x0[idx_split_rff][-1],label='RFF='+str(format(x0[idx_split_rff][-1],'.3f')),color='black')
plt.legend()
plt.xlabel('RFF')
plt.ylabel('Objetos')
# plt.show()
# plt.savefig(f'{sample}_stats_observation/plano_rff_eta/histogram_rff_lines_elip.png')
plt.close(fig0)
...
plt.ylabel('Objetos')
# plt.show()
# plt.savefig(f'{sample}_stats_observation/plano_rff_eta/histogram_rff_lines_e_el.png')
plt.close(fig0)
...
plt.ylabel('Objetos')
# plt.show()
# plt.savefig(f'{sample}_stats_observation/plano_rff_eta/histogram_rff_lines_true_cd.png')
plt.close(fig0)
'''
#########################################
#PLANO DE RFF - ETA
...
plt.ylabel(r'$\eta$')
# plt.show()
# plt.savefig(f'{sample}_stats_observation/plano_rff_eta/rffxeta_color_coded.png')
plt.close()
...
axs[2].legend()
# plt.show()
# plt.savefig(f'{sample}_stats_observation/plano_rff_eta/rffxeta_sub_grupos.png')
plt.close()
...
# pLt.show()
# plt.savefig(f'{sample}_stats_observation/plano_rff_eta/rffxeta_elipticas.png')
plt.close()
...
axs.legend()
# pLt.show()
# plt.savefig(f'{sample}_stats_observation/plano_rff_eta/rffxeta_true_cds.png')
plt.close()
...
axs.set_xlabel(r'$\log\,RFF$')
# pLt.show()
# plt.savefig(f'{sample}_stats_observation/plano_rff_eta/rffxeta_extra_light.png')
plt.close()

######@##########################################
#MAPA DE COR - RAZÃO BT - SEM CORREÇÃO
os.makedirs(f'{sample}_stats_observation/plano_rff_eta/bt',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
vec=[0.1,0.5,0.9]
for item in vec:
	plt.plot(np.log10(x),x-item*x,label=str(item))
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt/rffxeta_color_bt.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt/rffxeta_color_bt_elip.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt/rffxeta_color_bt_true_cd.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt/rffxeta_color_bt_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt/rffxeta_color_bt_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/bt/rff_eta_bt_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt/rffxeta_color_bt_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt/rffxeta_color_bt_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/bt/rff_eta_bt_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt/rffxeta_color_bt_3d_projections_only2c.png')
plt.close()

##################################
#MAPA DE COR - RAZÃO BT - CORRIGIDO
os.makedirs(f'{sample}_stats_observation/plano_rff_eta/bt_corr',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt_corr/rffxeta_color_bt_corr.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt_corr/rffxeta_color_bt_true_cd_corr.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt_corr/rffxeta_color_bt_elip_corr.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt_corr/rffxeta_color_bt_e_el_corr.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt_corr/rffxeta_color_bt_corr_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/bt_corr/rff_eta_bt_corr_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt_corr/rffxeta_color_bt_corr_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt_corr/rffxeta_color_bt_corr_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/bt_corr/rff_eta_bt_corr_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/bt_corr/rffxeta_color_bt_corr_3d_projections_only2c.png')
plt.close()

###################################
#MAPA DE RAZÃO DE RFF
os.makedirs(f'{sample}_stats_observation/plano_rff_eta/rff_ratio',exist_ok=True)
fig1=plt.figure(figsize=(9,7))

plt.scatter(np.log10(rff_s),eta,c=rff_ratio,edgecolors='black',cmap=cmap)
plt.axvline(np.log10(x0[idx_split_rff][-1]),label='RFF='+str(format(x0[idx_split_rff][-1],'.3f')),color='black')
cbar=plt.colorbar()
cbar.set_label(r'$RFF_{S+S}^2/RFF_{S}^2$', rotation=90)
plt.clim(0.,1.)
plt.legend()
plt.xlim(limx)
plt.ylim(limy)
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/rff_ratio/rffxeta_color_rff_ratio.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/rff_ratio/rffxeta_color_rff_ratio_elip.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/rff_ratio/rffxeta_color_rff_ratio_true_cd.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/rff_ratio/rffxeta_color_rff_ratio_e_el.png')
plt.close()
...
ax.set_xlabel(r'$\log\,RFF$')
ax.set_ylabel(r'$\eta$')
ax.set_zlabel(r'$RFF_{S+S}^2/RFF_{S}^2$')
ax.view_init(elev=20, azim=45)
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/rff_ratio/rffxeta_color_rff_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/rff_ratio/rff_eta_rff_ratio_3d_spin.gif', dpi=100):
...
axs[1].set_xlim([0,1])
axs[1].set_ylim(limx)
axs[1].set_xlabel(r'$RFF_{S+S}^2/RFF_{S}^2$')
axs[1].set_ylabel(r'$\log\,RFF$')
axs[1].legend()
#ZY
...
axs[2].set_xlim([0,1])
axs[2].set_ylim(limy)
axs[2].set_xlabel(r'$RFF_{S+S}^2/RFF_{S}^2$')
axs[2].set_ylabel(r'$\eta$')
axs[2].legend()

plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/rff_ratio/rffxeta_color_rff_ratio_3d_projections.png')
plt.close()

#################################
#MAPA DE COR RAZÃO DE CHI2
os.makedirs(f'{sample}_stats_observation/plano_rff_eta/chi2_ratio',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.xlabel(r'$\log\,RFF$')
plt.ylabel(r'$\eta$')
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/chi2_ratio/rffxeta_color_chi2_ratio.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/chi2_ratio/rffxeta_color_chi2_ratio_elip.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/chi2_ratio/rffxeta_color_chi2_ratio_true_cd.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/chi2_ratio/rffxeta_color_chi2_ratio_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/chi2_ratio/rffxeta_color_chi2_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/chi2_ratio/rff_eta_chi2_ratio_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/chi2_ratio/rffxeta_color_chi2_ratio_3d_projections.png')
plt.close()
#################################
#MAPA DE COR DELTA BIC
os.makedirs(f'{sample}_stats_observation/plano_rff_eta/delta_bic',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/delta_bic/rffxeta_color_delta_bic.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/delta_bic/rffxeta_color_delta_bic_elip.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/delta_bic/rffxeta_color_delta_bic_true_cd.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/delta_bic/rffxeta_color_delta_bic_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/delta_bic/rffxeta_color_delta_bic_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/delta_bic/rff_eta_delta_bic_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/delta_bic/rffxeta_color_delta_bic_3d_projections.png')
plt.close()
###############################
#MAPA DE COR RAZÃO AXIAL
os.makedirs(f'{sample}_stats_observation/plano_rff_eta/ax_ratio',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio/rffxeta_color_q.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio/rffxeta_color_q_elip.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio/rffxeta_color_q_true_cd.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio/rffxeta_color_q_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio/rffxeta_color_ax_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/ax_ratio/rff_eta_ax_ratio_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio/rffxeta_color_ax_ratio_3d_projections.png')
plt.close()

###############################
#MAPA DE COR RAZÃO - Q1/Q2
os.makedirs(f'{sample}_stats_observation/plano_rff_eta/q1q2',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/q1q2/rffxeta_color_q_ratio.png')
plt.close()
...
plt.scatter(np.log10(rff_s)[elip_lim],eta[elip_lim],c=axrat_ratio_e,edgecolors='black',label='E',cmap=cmap)
plt.axvline(np.log10(x0[idx_split_rff][-1]),label='RFF='+str(format(x0[idx_split_rff][-1],'.3f')),color='black')
cbar=plt.colorbar()
cbar.set_label(r'$q_1/q_2$', rotation=90)
plt.clim(0.7,2.)
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/q1q2/rffxeta_color_q_ratio_elip.png')
plt.close()
...
plt.scatter(np.log10(rff_s)[lim_cd_big],eta[lim_cd_big],c=axrat_ratio_cd_big,edgecolors='black',label='True cD',cmap=cmap)
plt.axvline(np.log10(x0[idx_split_rff][-1]),label='RFF='+str(format(x0[idx_split_rff][-1],'.3f')),color='black')
cbar=plt.colorbar()
cbar.set_label(r'$q$', rotation=90)
plt.clim(0.7,2.)
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/q1q2/rffxeta_color_q_ratio_true_cd.png')
plt.close()
...
plt.scatter(np.log10(rff_s)[lim_cd_small],eta[lim_cd_small],c=axrat_ratio_cd_small,edgecolors='black',label='True cD',cmap=cmap)
plt.axvline(np.log10(x0[idx_split_rff][-1]),label='RFF='+str(format(x0[idx_split_rff][-1],'.3f')),color='black')
cbar=plt.colorbar()
cbar.set_label(r'$q_1/q_2$', rotation=90)
plt.clim(0.7,2.)
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/q1q2/rffxeta_color_q_ratio_e_el.png')
plt.close()
...
ax.scatter(np.log10(rff_s[elip_lim]),eta[elip_lim],axrat_ratio_e,alpha=0.4,c='green',edgecolor='black',label='E')
ax.scatter(np.log10(rff_s[lim_cd_small]),eta[lim_cd_small],axrat_ratio_cd_small,alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
ax.scatter(np.log10(rff_s[lim_cd_big]),eta[lim_cd_big],axrat_ratio_cd_big,alpha=0.6,c='red',edgecolor='black',label='True cD')
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/q1q2/rffxeta_color_q_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/q1q2/rff_eta_q_ratio_3d_spin.gif', dpi=100):
...
axs[0].legend()
#ZX
axs[1].scatter(axrat_ratio_e,np.log10(rff_s[elip_lim]),alpha=0.4,c='green',edgecolor='black',label='E')
axs[1].scatter(axrat_ratio_cd_small,np.log10(rff_s[lim_cd_small]),alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[1].scatter(axrat_ratio_cd_big,np.log10(rff_s[lim_cd_big]),alpha=0.6,c='red',edgecolor='black',label='True cD')
...
axs[1].legend()
#ZY
axs[2].scatter(axrat_ratio_e,eta[elip_lim],alpha=0.4,c='green',edgecolor='black',label='E')
axs[2].scatter(axrat_ratio_cd_small,eta[lim_cd_small],alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[2].scatter(axrat_ratio_cd_big,eta[lim_cd_big],alpha=0.6,c='red',edgecolor='black',label='True cD')
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/q1q2/rffxeta_color_q_ratio_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/q1q2/rffxeta_color_q_ratio_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/q1q2/rff_eta_q_ratio_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/q1q2/rffxeta_color_q_ratio_3d_projections_only2c.png')
plt.close()
###############################
#MAPA DE COR BOXINESS
os.makedirs(f'{sample}_stats_observation/plano_rff_eta/box',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/box/rffxeta_color_box_s.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/box/rffxeta_color_box_elip.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/box/rffxeta_color_box_true_cd.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/box/rffxeta_color_box_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/box/rffxeta_color_box_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/box/rff_eta_box_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/box/rffxeta_color_box_3d_projections.png')
plt.close()

###############################
#MAPA DE COR RAZÃO - re1/re2
os.makedirs(f'{sample}_stats_observation/plano_rff_eta/re1re2',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/re1re2/rffxeta_color_re_ratio.png')
plt.close()
...
plt.scatter(np.log10(rff_s)[elip_lim],eta[elip_lim],c=np.log10(re_ratio_e),edgecolors='black',label='E',cmap=cmap)
plt.axvline(np.log10(x0[idx_split_rff][-1]),label='RFF='+str(format(x0[idx_split_rff][-1],'.3f')),color='black')
cbar=plt.colorbar()
cbar.set_label(r'$\log_{10}({R_e}_{1}/{R_e}_{2})$', rotation=90)
plt.clim(-2.,0.1)
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/re1re2/rffxeta_color_re_ratio_elip.png')
plt.close()
...
plt.scatter(np.log10(rff_s)[lim_cd_big],eta[lim_cd_big],c=np.log10(re_ratio_cd_big),edgecolors='black',label='True cD',cmap=cmap)
plt.axvline(np.log10(x0[idx_split_rff][-1]),label='RFF='+str(format(x0[idx_split_rff][-1],'.3f')),color='black')
cbar=plt.colorbar()
cbar.set_label(r'$\log_{10}({R_e}_{1}/{R_e}_{2})$', rotation=90)
plt.clim(-2.,0.1)
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/re1re2/rffxeta_color_re_ratio_true_cd.png')
plt.close()
...
plt.scatter(np.log10(rff_s)[lim_cd_small],eta[lim_cd_small],c=np.log10(re_ratio_cd_small),edgecolors='black',label='True cD',cmap=cmap)
plt.axvline(np.log10(x0[idx_split_rff][-1]),label='RFF='+str(format(x0[idx_split_rff][-1],'.3f')),color='black')
cbar=plt.colorbar()
cbar.set_label(r'$\log_{10}{R_e}_{1}/{R_e}_{2}$', rotation=90)
plt.clim(-2.,0.1)
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/re1re2/rffxeta_color_re_ratio_e_el.png')
plt.close()
...
ax.scatter(np.log10(rff_s[elip_lim]),eta[elip_lim],np.log10(re_ratio_e),alpha=0.4,c='green',edgecolor='black',label='E')
ax.scatter(np.log10(rff_s[lim_cd_small]),eta[lim_cd_small],np.log10(re_ratio_cd_small),alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
ax.scatter(np.log10(rff_s[lim_cd_big]),eta[lim_cd_big],np.log10(re_ratio_cd_big),alpha=0.6,c='red',edgecolor='black',label='True cD')
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/re1re2/rffxeta_color_re_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/re1re2/rff_eta_re_ratio_3d_spin.gif', dpi=100):
...
axs[0].legend()
#ZX
axs[1].scatter(np.log10(re_ratio_e),np.log10(rff_s[elip_lim]),alpha=0.4,c='green',edgecolor='black',label='E')
axs[1].scatter(np.log10(re_ratio_cd_small),np.log10(rff_s[lim_cd_small]),alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[1].scatter(np.log10(re_ratio_cd_big),np.log10(rff_s[lim_cd_big]),alpha=0.6,c='red',edgecolor='black',label='True cD')
axs[1].set_xlim([-2,1])
axs[1].set_ylim(limx)
...
axs[1].legend()
#ZY
axs[2].scatter(np.log10(re_ratio_e),eta[elip_lim],alpha=0.4,c='green',edgecolor='black',label='E')
axs[2].scatter(np.log10(re_ratio_cd_small),eta[lim_cd_small],alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[2].scatter(np.log10(re_ratio_cd_big),eta[lim_cd_big],alpha=0.6,c='red',edgecolor='black',label='True cD')
axs[2].set_xlim([-2,1])
axs[2].set_ylim(limy)
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/re1re2/rffxeta_color_q_ratio_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/re1re2/rffxeta_color_re_ratio_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/re1re2/rff_eta_re_ratio_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/re1re2/rffxeta_color_re_ratio_3d_projections_only2c.png')
plt.close()

################################
#MAPA DE COR RAZÃO - n1/n2
os.makedirs(f'{sample}_stats_observation/plano_rff_eta/n1n2',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1n2/rffxeta_color_n_ratio.png')
plt.close()
...
plt.scatter(np.log10(rff_s)[elip_lim],eta[elip_lim],c=n_ratio_e,edgecolors='black',label='E',cmap=cmap)
plt.axvline(np.log10(x0[idx_split_rff][-1]),label='RFF='+str(format(x0[idx_split_rff][-1],'.3f')),color='black')
cbar=plt.colorbar()
cbar.set_label(r'${n}_{1}/{n}_{2}$', rotation=90)
plt.clim(0,2.)
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1n2/rffxeta_color_n_ratio_elip.png')
plt.close()
...
plt.scatter(np.log10(rff_s)[lim_cd_big],eta[lim_cd_big],c=n_ratio_cd_big,edgecolors='black',label='True cD',cmap=cmap)
plt.axvline(np.log10(x0[idx_split_rff][-1]),label='RFF='+str(format(x0[idx_split_rff][-1],'.3f')),color='black')
cbar=plt.colorbar()
cbar.set_label(r'${n}_{1}/{n}_{2}$', rotation=90)
plt.clim(0,2.)
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1n2/rffxeta_color_n_ratio_true_cd.png')
plt.close()
...
plt.scatter(np.log10(rff_s)[lim_cd_small],eta[lim_cd_small],c=n_ratio_cd_small,edgecolors='black',label='True cD',cmap=cmap)
plt.axvline(np.log10(x0[idx_split_rff][-1]),label='RFF='+str(format(x0[idx_split_rff][-1],'.3f')),color='black')
cbar=plt.colorbar()
cbar.set_label(r'${n}_{1}/{n}_{2}$', rotation=90)
plt.clim(0,2.)
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1n2/rffxeta_color_n_ratio_e_el.png')
plt.close()
...
ax.scatter(np.log10(rff_s[elip_lim]),eta[elip_lim],n_ratio_e,alpha=0.4,c='green',edgecolor='black',label='E')
ax.scatter(np.log10(rff_s[lim_cd_small]),eta[lim_cd_small],n_ratio_cd_small,alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
ax.scatter(np.log10(rff_s[lim_cd_big]),eta[lim_cd_big],n_ratio_cd_big,alpha=0.6,c='red',edgecolor='black',label='True cD')
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1n2/rffxeta_color_n_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/n1n2/rff_eta_n_ratio_3d_spin.gif', dpi=100):
...
axs[0].legend()
#ZX
axs[1].scatter(n_ratio_e,np.log10(rff_s[elip_lim]),alpha=0.4,c='green',edgecolor='black',label='E')
axs[1].scatter(n_ratio_cd_small,np.log10(rff_s[lim_cd_small]),alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[1].scatter(n_ratio_cd_big,np.log10(rff_s[lim_cd_big]),alpha=0.6,c='red',edgecolor='black',label='True cD')
axs[1].set_xlim([0,5.])
axs[1].set_ylim(limx)
...
axs[1].legend()
#ZY
axs[2].scatter(n_ratio_e,eta[elip_lim],alpha=0.4,c='green',edgecolor='black',label='E')
axs[2].scatter(n_ratio_cd_small,eta[lim_cd_small],alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[2].scatter(n_ratio_cd_big,eta[lim_cd_big],alpha=0.6,c='red',edgecolor='black',label='True cD')
axs[2].set_xlim([0,5])
axs[2].set_ylim(limy)
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1n2/rffxeta_color_n_ratio_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1n2/rffxeta_color_n_ratio_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/n1n2/rff_eta_n_ratio_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1n2/rffxeta_color_n_ratio_3d_projections_only2c.png')
plt.close()

################################
#MAPA DE COR - INDICE DE SÉRSIC
os.makedirs(f'{sample}_stats_observation/plano_rff_eta/n_sersic',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n_sersic/rffxeta_color_n.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n_sersic/rffxeta_color_n_elip.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n_sersic/rffxeta_color_n_true_cd.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n_sersic/rffxeta_color_n_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n_sersic/rffxeta_color_n_3d.png')
plt.close()
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n_sersic/rffxeta_color_n_3d_projections.png')
plt.close()
##########################
#MAPA DE COR - INDICE DE SÉRSIC 1 - SS
os.makedirs(f'{sample}_stats_observation/plano_rff_eta/n1',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1/rffxeta_color_n1.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1/rffxeta_color_n1_elip.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1/rffxeta_color_n1_true_cd.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1/rffxeta_color_n1_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1/rffxeta_color_n1_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/n1/rff_eta_n1_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1/rffxeta_color_n1_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1/rffxeta_color_n1_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/n1/rff_eta_n1_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n1/rffxeta_color_n1_3d_projections_only2c.png')
plt.close()
##########################
#MAPA DE COR - INDICE DE SÉRSIC 2 - SS
os.makedirs(f'{sample}_stats_observation/plano_rff_eta/n2',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n2/rffxeta_color_n2.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n2/rffxeta_color_n2_elip.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n2/rffxeta_color_n2_true_cd.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n2/rffxeta_color_n2_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n2/rffxeta_color_n2_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/n2/rff_eta_n2_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n2/rffxeta_color_n2_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n2/rffxeta_color_n2_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/n2/rff_eta_n2_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/n2/rffxeta_color_n2_3d_projections_only2c.png')
plt.close()
##########################
#MAPA DE COR - RAZÃO AXIAL 1 - SS
os.makedirs(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_1',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_1/rffxeta_color_q1.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_1/rffxeta_color_q1_elip.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_1/rffxeta_color_q1_true_cd.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_1/rffxeta_color_q1_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_1/rffxeta_color_q1_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/ax_ratio_1/rff_eta_q1_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_1/rffxeta_color_q1_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_1/rffxeta_color_q1_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/ax_ratio_1/rff_eta_q1_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_1/rffxeta_color_q1_3d_projections_only2c.png')
plt.close()
##########################
#MAPA DE COR - RAZÃO AXIAL 2 - SS
os.makedirs(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_2',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_2/rffxeta_color_q2.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_2/rffxeta_color_q2_elip.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_2/rffxeta_color_q2_true_cd.png')
plt.close()
...
plt.ylabel(r'$\eta$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_2/rffxeta_color_q2_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_2/rffxeta_color_q2_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/ax_ratio_2/rff_eta_q2_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_2/rffxeta_color_q2_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_2/rffxeta_color_q2_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_rff_eta/ax_ratio_2/rff_eta_q2_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_rff_eta/ax_ratio_2/rffxeta_color_q2_3d_projections_only2c.png')
plt.close()
'''
##########################################
...
# # ax.scatter(np.log10(rff_s)[elip_lim],bt_vec_corr[elip_lim],rff_ratio[elip_lim],c='red',edgecolor='black',label='E')
# # ax.scatter(np.log10(re_ratio_12)[elip_lim],n2[elip_lim],bt_vec_corr[elip_lim],edgecolors='black',color='green')
# ax.scatter(np.log10(re_ratio_12[lim_cd_big]),(n_ratio_12[lim_cd_big]),(bt_vec_corr[lim_cd_big]),edgecolors='black',color='red')
# ax.scatter(np.log10(re_ratio_12[lim_cd_small]),(n_ratio_12[lim_cd_small]),(bt_vec_corr[lim_cd_small]),edgecolors='black',color='blue')
# # ax.scatter(np.log10(rff_s)[cd_lim],bt_vec_corr[lim],rff_ratio[cd_lim],c='blue',edgecolor='black',label='2C')
# # ax.scatter(np.log10(n_ratio_12)[lim_cd_big & lim_photutils],n2[lim_cd_big & lim_photutils],med_a3[lim_cd_big[lim_photutils]],c='red',edgecolor='black',label='True cD')
...
# re_ratio_cd,re_ratio_cd_small,re_ratio_cd_big,re_ratio_e
label_x=r'$R_{1}/R_{2}$'
label_y=r'$n_{2}$'
...
axs[2].legend()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/scatter_re_ratio_n2_geral.png')
plt.close()
...
axs[2].set_xlabel(label_x)
axs[2].legend()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/rexn2_sub_grupos.png')
plt.close()
...
axs.set_xlabel(label_x)
axs.legend()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/rexn2_elipticas.png')
plt.close()
...
axs.set_xlabel(label_x)
axs.legend()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/rexn2_true_cds.png')
plt.close()
...
axs.set_ylabel(label_y)
axs.set_xlabel(label_x)
plt.savefig(f'{sample}_stats_observation/plano_re_n2/rexn2_extra_light.png')
plt.close()

################################################
#MAPA DE COR - RAZÃO BT - SEM CORREÇÃO
os.makedirs(f'{sample}_stats_observation/plano_re_n2/bt',exist_ok=True)
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt/rexn2_color_bt.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt/rexn2_color_bt_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt/rexn2_color_bt_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt/rexn2_color_bt_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt/rexn2_color_bt_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/bt/re_n2_bt_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt/rexn2_color_bt_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt/rexn2_color_bt_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/bt/re_n2_bt_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt/rexn2_color_bt_3d_projections_only2c.png')
plt.close()

##################################
#MAPA DE COR - RAZÃO BT - CORRIGIDO
os.makedirs(f'{sample}_stats_observation/plano_re_n2/bt_corr',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
plt.suptitle('BT CORRIGIDO')
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt_corr/rexn2_color_bt_corr.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt_corr/rexn2_color_bt_true_cd_corr.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt_corr/rexn2_color_bt_elip_corr.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt_corr/rexn2_color_bt_e_el_corr.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt_corr/rexn2_color_bt_corr_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/bt_corr/re_n2_bt_corr_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt_corr/rexn2_color_bt_corr_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt_corr/rexn2_color_bt_corr_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/bt_corr/re_n2_bt_corr_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/bt_corr/rexn2_color_bt_corr_3d_projections_only2c.png')
plt.close()
###################################
#MAPA DE RAZÃO DE RFF
os.makedirs(f'{sample}_stats_observation/plano_re_n2/rff_ratio',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/rff_ratio/rexn2_color_rff_ratio.png')
plt.close()
...
plt.scatter(np.log10(re_ratio_12)[elip_lim],n2[elip_lim],c=rff_ratio_e,edgecolors='black',label='E',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'$RFF_{S+S}^2/RFF_{S}^2$', rotation=90)
plt.clim(0.,1.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/rff_ratio/rexn2_color_rff_ratio_elip.png')
plt.close()

#MAPA DE RAZÃO DE RFF - TRUE cDS

fig1=plt.figure(figsize=(9,7))
plt.scatter(np.log10(re_ratio_12)[lim_cd_big],n2[lim_cd_big],c=rff_ratio_cd_big,edgecolors='black',label='True cD',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'$RFF_{S+S}^2/RFF_{S}^2$', rotation=90)
plt.clim(0.,1.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/rff_ratio/rexn2_color_rff_ratio_true_cd.png')
plt.close()

#MAPA DE RAZÃO DE RFF - TRUE cDS

fig1=plt.figure(figsize=(9,7))
plt.scatter(np.log10(re_ratio_12)[lim_cd_small],n2[lim_cd_small],c=rff_ratio_cd_small,edgecolors='black',label='E(EL)',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'$RFF_{S+S}^2/RFF_{S}^2$', rotation=90)
plt.clim(0.,1.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/rff_ratio/rexn2_color_rff_ratio_e_el.png')
plt.close()
...
fig = plt.figure(figsize=(8, 7))
ax = fig.add_subplot(1,1,1,projection='3d')
ax.scatter(np.log10(re_ratio_12[elip_lim]),n2[elip_lim],rff_ratio_e,alpha=0.4,c='green',edgecolor='black',label='E')
ax.scatter(np.log10(re_ratio_12[lim_cd_small]),n2[lim_cd_small],rff_ratio_cd_small,alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
ax.scatter(np.log10(re_ratio_12[lim_cd_big]),n2[lim_cd_big],rff_ratio_cd_big,alpha=0.6,c='red',edgecolor='black',label='True cD')
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/rff_ratio/rexn2_color_rff_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/rff_ratio/re_n2_rff_ratio_3d_spin.gif', dpi=100):
...
axs[0].legend()
#ZX
axs[1].scatter(rff_ratio_e,np.log10(re_ratio_12[elip_lim]),alpha=0.4,c='green',edgecolor='black',label='E')
axs[1].scatter(rff_ratio_cd_small,np.log10(re_ratio_12[lim_cd_small]),alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[1].scatter(rff_ratio_cd_big,np.log10(re_ratio_12[lim_cd_big]),alpha=0.6,c='red',edgecolor='black',label='True cD')
axs[1].set_xlim([0,1])
axs[1].set_xlabel(r'$RFF_{S+S}^2/RFF_{S}^2$')
axs[1].set_ylabel(label_x)
axs[1].legend()
#ZY
axs[2].scatter(rff_ratio_e,n2[elip_lim],alpha=0.4,c='green',edgecolor='black',label='E')
axs[2].scatter(rff_ratio_cd_small,n2[lim_cd_small],alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[2].scatter(rff_ratio_cd_big,n2[lim_cd_big],alpha=0.6,c='red',edgecolor='black',label='True cD')
axs[2].set_xlim([0,1])
axs[2].set_xlabel(r'$RFF_{S+S}^2/RFF_{S}^2$')
axs[2].set_ylabel(label_y)
axs[2].legend()
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/rff_ratio/rexn2_color_rff_ratio_3d_projections.png')
plt.close()

#################################
#MAPA DE COR RAZÃO DE CHI2
os.makedirs(f'{sample}_stats_observation/plano_re_n2/chi2_ratio',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.xlabel(label_x)
plt.ylabel(label_y)
plt.savefig(f'{sample}_stats_observation/plano_re_n2/chi2_ratio/rexn2_color_chi2_ratio.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/chi2_ratio/rexn2_color_chi2_ratio_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/chi2_ratio/rexn2_color_chi2_ratio_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/chi2_ratio/rexn2_color_chi2_ratio_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/chi2_ratio/rexn2_color_chi2_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/chi2_ratio/re_n2_chi2_ratio_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/chi2_ratio/rexn2_color_chi2_ratio_3d_projections.png')
plt.close()

#################################
#MAPA DE COR DELTA BIC
os.makedirs(f'{sample}_stats_observation/plano_re_n2/delta_bic',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/delta_bic/rexn2_color_delta_bic.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/delta_bic/rexn2_color_delta_bic_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/delta_bic/rexn2_color_delta_bic_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/delta_bic/rexn2_color_delta_bic_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/delta_bic/rexn2_color_delta_bic_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/delta_bic/re_n2_delta_bic_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/delta_bic/rexn2_color_delta_bic_3d_projections.png')
plt.close()

###############################
#MAPA DE COR RAZÃO AXIAL
os.makedirs(f'{sample}_stats_observation/plano_re_n2/ax_ratio',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio/rexn2_color_q.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio/rexn2_color_q_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio/rexn2_color_q_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio/rexn2_color_q_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio/rexn2_color_ax_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/ax_ratio/re_n2_ax_ratio_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio/rexn2_color_ax_ratio_3d_projections.png')
plt.close()

###############################
#MAPA DE COR RAZÃO - Q1/Q2
os.makedirs(f'{sample}_stats_observation/plano_re_n2/q1q2',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/q1q2/rexn2_color_q_ratio.png')
plt.close()
...
plt.scatter(np.log10(re_ratio_12)[elip_lim],n2[elip_lim],c=axrat_ratio_e,edgecolors='black',label='E',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'$q_1/q_2$', rotation=90)
plt.clim(0.7,2.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/q1q2/rexn2_color_q_ratio_elip.png')
plt.close()
...
fig1=plt.figure(figsize=(9,7))
plt.scatter(np.log10(re_ratio_12)[lim_cd_big],n2[lim_cd_big],c=axrat_ratio_cd_big,edgecolors='black',label='True cD',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'$q$', rotation=90)
plt.clim(0.7,2.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/q1q2/rexn2_color_q_ratio_true_cd.png')
plt.close()

#MAPA DE COR RAZÃO Q1/Q2 - EXTRA LIGHT

fig1=plt.figure(figsize=(9,7))
plt.scatter(np.log10(re_ratio_12)[lim_cd_small],n2[lim_cd_small],c=axrat_ratio_cd_small,edgecolors='black',label='True cD',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'$q_1/q_2$', rotation=90)
plt.clim(0.7,2.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/q1q2/rexn2_color_q_ratio_e_el.png')
plt.close()
...
fig = plt.figure(figsize=(8, 7))
ax = fig.add_subplot(1,1,1,projection='3d')
ax.scatter(np.log10(re_ratio_12[elip_lim]),n2[elip_lim],axrat_ratio_e,alpha=0.4,c='green',edgecolor='black',label='E')
ax.scatter(np.log10(re_ratio_12[lim_cd_small]),n2[lim_cd_small],axrat_ratio_cd_small,alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
ax.scatter(np.log10(re_ratio_12[lim_cd_big]),n2[lim_cd_big],axrat_ratio_cd_big,alpha=0.6,c='red',edgecolor='black',label='True cD')
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/q1q2/rexn2_color_q_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/q1q2/re_n2_q_ratio_3d_spin.gif', dpi=100):
...
axs[0].legend()
#ZX
axs[1].scatter(axrat_ratio_e,np.log10(re_ratio_12[elip_lim]),alpha=0.4,c='green',edgecolor='black',label='E')
axs[1].scatter(axrat_ratio_cd_small,np.log10(re_ratio_12[lim_cd_small]),alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[1].scatter(axrat_ratio_cd_big,np.log10(re_ratio_12[lim_cd_big]),alpha=0.6,c='red',edgecolor='black',label='True cD')
axs[1].set_xlim([0.1,2.8])
axs[1].set_xlabel(r'$q_1/q_2$')
axs[1].set_ylabel(label_x)
axs[1].legend()
#ZY
axs[2].scatter(axrat_ratio_e,n2[elip_lim],alpha=0.4,c='green',edgecolor='black',label='E')
axs[2].scatter(axrat_ratio_cd_small,n2[lim_cd_small],alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[2].scatter(axrat_ratio_cd_big,n2[lim_cd_big],alpha=0.6,c='red',edgecolor='black',label='True cD')
axs[2].set_xlim([0.1,2.8])
axs[2].set_xlabel(r'$q_1/q_2$')
axs[2].set_ylabel(label_y)
axs[2].legend()
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/q1q2/rexn2_color_q_ratio_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/q1q2/rexn2_color_q_ratio_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/q1q2/re_n2_q_ratio_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/q1q2/rexn2_color_q_ratio_3d_projections_only2c.png')
plt.close()

###############################
#MAPA DE COR BOXINESS
os.makedirs(f'{sample}_stats_observation/plano_re_n2/box',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/box/rexn2_color_box_s.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/box/rexn2_color_box_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/box/rexn2_color_box_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/box/rexn2_color_box_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/box/rexn2_color_box_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/box/re_n2_box_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/box/rexn2_color_box_3d_projections.png')
plt.close()

################################
#MAPA DE COR RAZÃO - n1/n2
os.makedirs(f'{sample}_stats_observation/plano_re_n2/n1n2',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1n2/rexn2_color_n_ratio.png')
plt.close()
...
plt.scatter(np.log10(re_ratio_12)[elip_lim],n2[elip_lim],c=n_ratio_e,edgecolors='black',label='E',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'${n}_{1}/{n}_{2}$', rotation=90)
plt.clim(0,2.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1n2/rexn2_color_n_ratio_elip.png')
plt.close()

#MAPA DE COR RAZÃO n1/n2 - TRUE cDS

fig1=plt.figure(figsize=(9,7))
plt.scatter(np.log10(re_ratio_12)[lim_cd_big],n2[lim_cd_big],c=n_ratio_cd_big,edgecolors='black',label='True cD',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'${n}_{1}/{n}_{2}$', rotation=90)
plt.clim(0,2.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1n2/rexn2_color_n_ratio_true_cd.png')
plt.close()

#MAPA DE COR RAZÃO n1/n2 - EXTRA LIGHT

fig1=plt.figure(figsize=(9,7))
plt.scatter(np.log10(re_ratio_12)[lim_cd_small],n2[lim_cd_small],c=n_ratio_cd_small,edgecolors='black',label='True cD',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'${n}_{1}/{n}_{2}$', rotation=90)
plt.clim(0,2.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1n2/rexn2_color_n_ratio_e_el.png')
plt.close()
...
ax.scatter(np.log10(re_ratio_12[elip_lim]),n2[elip_lim],n_ratio_e,alpha=0.4,c='green',edgecolor='black',label='E')
ax.scatter(np.log10(re_ratio_12[lim_cd_small]),n2[lim_cd_small],n_ratio_cd_small,alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
ax.scatter(np.log10(re_ratio_12[lim_cd_big]),n2[lim_cd_big],n_ratio_cd_big,alpha=0.6,c='red',edgecolor='black',label='True cD')
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1n2/rexn2_color_n_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/n1n2/re_n2_n_ratio_3d_spin.gif', dpi=100):
...
axs[0].legend()
#ZX
axs[1].scatter(n_ratio_e,np.log10(re_ratio_12[elip_lim]),alpha=0.4,c='green',edgecolor='black',label='E')
axs[1].scatter(n_ratio_cd_small,np.log10(re_ratio_12[lim_cd_small]),alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[1].scatter(n_ratio_cd_big,np.log10(re_ratio_12[lim_cd_big]),alpha=0.6,c='red',edgecolor='black',label='True cD')
axs[1].set_xlim([0,5.])
axs[1].set_xlabel(r'$n_1/n_2$')
axs[1].set_ylabel(label_x)
axs[1].legend()
#ZY
axs[2].scatter(n_ratio_e,n2[elip_lim],alpha=0.4,c='green',edgecolor='black',label='E')
axs[2].scatter(n_ratio_cd_small,n2[lim_cd_small],alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[2].scatter(n_ratio_cd_big,n2[lim_cd_big],alpha=0.6,c='red',edgecolor='black',label='True cD')
axs[2].set_xlim([0,5])
axs[2].set_xlabel(r'$n_1/n_2$')
axs[2].set_ylabel(label_y)
axs[2].legend()
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1n2/rexn2_color_n_ratio_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1n2/rexn2_color_n_ratio_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/n1n2/re_n2_n_ratio_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1n2/rexn2_color_n_ratio_3d_projections_only2c.png')
plt.close()

################################
#MAPA DE COR - INDICE DE SÉRSIC
os.makedirs(f'{sample}_stats_observation/plano_re_n2/n_sersic',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n_sersic/rexn2_color_n.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n_sersic/rexn2_color_n_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n_sersic/rexn2_color_n_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n_sersic/rexn2_color_n_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n_sersic/rexn2_color_n_3d.png')
plt.close()
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n_sersic/rexn2_color_n_3d_projections.png')
plt.close()

##########################
#MAPA DE COR - INDICE DE SÉRSIC 1 - SS
os.makedirs(f'{sample}_stats_observation/plano_re_n2/n1',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1/rexn2_color_n1.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1/rexn2_color_n1_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1/rexn2_color_n1_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1/rexn2_color_n1_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1/rexn2_color_n1_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/n1/re_n2_n1_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1/rexn2_color_n1_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1/rexn2_color_n1_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/n1/re_n2_n1_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/n1/rexn2_color_n1_3d_projections_only2c.png')
plt.close()

##########################
#MAPA DE COR - RAZÃO AXIAL 1 - SS
os.makedirs(f'{sample}_stats_observation/plano_re_n2/ax_ratio_1',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_1/rexn2_color_q1.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_1/rexn2_color_q1_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_1/rexn2_color_q1_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_1/rexn2_color_q1_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_1/rexn2_color_q1_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/ax_ratio_1/re_n2_q1_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_1/rexn2_color_q1_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_1/rexn2_color_q1_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/ax_ratio_1/re_n2_q1_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_1/rexn2_color_q1_3d_projections_only2c.png')
plt.close()

##########################
#MAPA DE COR - RAZÃO AXIAL 2 - SS
os.makedirs(f'{sample}_stats_observation/plano_re_n2/ax_ratio_2',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_2/rexn2_color_q2.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_2/rexn2_color_q2_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_2/rexn2_color_q2_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_2/rexn2_color_q2_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_2/rexn2_color_q2_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/ax_ratio_2/re_n2_q2_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_2/rexn2_color_q2_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_2/rexn2_color_q2_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_re_n2/ax_ratio_2/re_n2_q2_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_re_n2/ax_ratio_2/rexeta_color_q2_3d_projections_only2c.png')
plt.close()
...
fig,axs=plt.subplots(1,3,figsize=(15,5),sharey=True,sharex=True,constrained_layout=True)
axs[0].scatter(np.log10(re_ratio_12)[elip_lim],n2[elip_lim],c='green',edgecolor='black',label='E')
axs[0].set_xlabel(r'$R_{1}/R_{2}$')
axs[0].set_ylabel(r'$n_{2}$')
axs[0].legend()
axs[1].scatter(np.log10(re_ratio_12)[lim_cd_small],n2[lim_cd_small],c='blue',edgecolor='black',label='E(EL)')
axs[1].set_xlabel(r'$R_{1}/R_{2}$')
axs[1].set_ylabel(r'$n_{2}$')
axs[1].legend()
axs[2].scatter(np.log10(re_ratio_12)[lim_cd_big],n2[lim_cd_big],c='red',edgecolor='black',label='True cD')
axs[2].set_xlabel(r'$R_{1}/R_{2}$')
axs[2].set_ylabel(r'$n_{2}$')
axs[2].legend()
# plt.show()
plt.savefig(f'{sample}_stats_observation/scatter_re_n/scatter_re_ratio_n2_geral.png')
plt.close()
...
fig,axs=plt.subplots(1,3,figsize=(15,5),sharey=True,sharex=True,constrained_layout=True)
sc=axs[0].scatter(np.log10(re_ratio_12)[elip_lim],n2[elip_lim],c=bt_vec_12[elip_lim],edgecolor='black',vmin=0,vmax=1,cmap=cmap,label='E')
axs[0].set_xlabel(r'$R_{1}/R_{2}$')
axs[0].set_ylabel(r'$n_{2}$')
axs[0].legend()
axs[1].scatter(np.log10(re_ratio_12)[lim_cd_small],n2[lim_cd_small],c=bt_vec_12[lim_cd_small],edgecolor='black',vmin=0,vmax=1,cmap=cmap,label='E(EL)')
axs[1].set_xlabel(r'$R_{1}/R_{2}$')
axs[1].set_ylabel(r'$n_{2}$')
axs[1].legend()
axs[2].scatter(np.log10(re_ratio_12)[lim_cd_big],n2[lim_cd_big],c=bt_vec_12[lim_cd_big],edgecolor='black',vmin=0,vmax=1,cmap=cmap,label='True cD')
axs[2].set_xlabel(r'$R_{1}/R_{2}$')
axs[2].set_ylabel(r'$n_{2}$')
axs[2].legend()
...
# plt.show()
plt.savefig(f'{sample}_stats_observation/scatter_re_n/scatter_re_ratio_n2_bt_geral.png')
plt.close()

fig,axs=plt.subplots(1,3,figsize=(15,5),sharey=True,sharex=True,constrained_layout=True)
sc=axs[0].scatter(np.log10(re_ratio_12)[elip_lim],n2[elip_lim],c=n1[elip_lim],edgecolor='black',cmap=cmap,vmax=5,label='E')
axs[0].set_xlabel(r'$R_{1}/R_{2}$')
axs[0].set_ylabel(r'$n_{2}$')
axs[0].legend()
axs[1].scatter(np.log10(re_ratio_12)[lim_cd_small],n2[lim_cd_small],c=n1[lim_cd_small],edgecolor='black',vmax=5,cmap=cmap,label='E(EL)')
axs[1].set_xlabel(r'$R_{1}/R_{2}$')
axs[1].set_ylabel(r'$n_{2}$')
axs[1].legend()
axs[2].scatter(np.log10(re_ratio_12)[lim_cd_big],n2[lim_cd_big],c=n1[lim_cd_big],edgecolor='black',vmax=5,cmap=cmap,label='True cD')
axs[2].set_xlabel(r'$R_{1}/R_{2}$')
axs[2].set_ylabel(r'$n_{2}$')
axs[2].legend()
cbar=plt.colorbar(sc,ax=axs[2])
...
# plt.show()
plt.savefig(f'{sample}_stats_observation/scatter_re_n/scatter_re_ratio_n2_n1_geral.png')
plt.close()

fig,axs=plt.subplots(1,3,figsize=(15,5),sharey=True,sharex=True,constrained_layout=True)
sc=axs[0].scatter(np.log10(re_ratio_12)[elip_lim],n2[elip_lim],c=rff_ratio[elip_lim],edgecolor='black',cmap=cmap,label='E')
axs[0].set_xlabel(r'$R_{1}/R_{2}$')
axs[0].set_ylabel(r'$n_{2}$')
axs[0].legend()
axs[1].scatter(np.log10(re_ratio_12)[lim_cd_small],n2[lim_cd_small],c=rff_ratio[lim_cd_small],edgecolor='black',cmap=cmap,label='E(EL)')
axs[1].set_xlabel(r'$R_{1}/R_{2}$')
axs[1].set_ylabel(r'$n_{2}$')
axs[1].legend()
axs[2].scatter(np.log10(re_ratio_12)[lim_cd_big],n2[lim_cd_big],c=rff_ratio[lim_cd_big],edgecolor='black',cmap=cmap,label='True cD')
axs[2].set_xlabel(r'$R_{1}/R_{2}$')
axs[2].set_ylabel(r'$n_{2}$')
axs[2].legend()
cbar=plt.colorbar(sc,ax=axs[2])
cbar.set_label(r'$RFF_{2C}/RFF_{1C}$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/scatter_re_n/scatter_re_ratio_n2_rff_ratio_geral.png')
plt.close()
...
# cbar.set_label(r'$B/T$')
# # plt.show()
# # plt.savefig(f'{sample}_stats_observation/scatter_re_ratio_n2_bt_12_geral.png')
# plt.close()
...
# plt.show()
plt.savefig('L07_stats_observation/scatter_re_ratio_n_1_n_2_geral.png')
plt.close()
...
# plt.show()
plt.savefig('L07_stats_observation/scatter_re_ratio_n_2_delta_bic_12_geral.png')
plt.close()
...
# plt.show()
plt.savefig('L07_stats_observation/scatter_re_ratio_n_2_n_ratio_12_geral.png')
plt.close()
...
# plt.show()
plt.savefig('L07_stats_observation/scatter_re_ratio_n_2_rff_12_geral.png')
plt.close()
...
# plt.show()
plt.savefig('L07_stats_observation/scatter_re_ratio_n_2_rff_ratio_12_geral.png')
plt.close()
...
# plt.show()
plt.savefig('L07_stats_observation/scatter_re_ratio_n_2_bt_12.png')
plt.close()
...
cbar.set_label(r'$n_{2}$')
# plt.show()
plt.savefig('L07_stats_observation/scatter_re_n_2_n_1.png')
plt.close()
...
cbar.set_label(r'$n_{1}/n_{2}$')
# plt.show()
plt.savefig('L07_stats_observation/scatter_re_n_2_n_ratio_12.png')
plt.close()
...
cbar.set_label(r'$\Delta BIC$')
# plt.show()
plt.savefig('L07_stats_observation/scatter_re_ratio_n_2_delta_bic_12.png')
plt.close()
...
# plt.show()
plt.savefig('L07_stats_observation/scatter_re_ratio_n_2_rff_s_12.png')
plt.close()
...
cbar.set_label(r'$RFF_{ss}/RFF_s$')
# plt.show()
plt.savefig('L07_stats_observation/scatter_re_ratio_n_2_rff_ratio_12.png')
plt.close()
'''
#############################################
...
'''
 ['bt_corr' 'q1q2' 're1re2' '0.8347683476834769']
 ['chi2_ratio' 'q1q2' 're1re2' '0.8331283312833129']
 ['n1' 'n_sersic' 'q1q2' '0.8343583435834359']
 ['n1' 'q1q2' 're1re2' '0.8306683066830668']
 ['q1q2' 're1re2' 'n2' '0.8355883558835588']
 ['q1q2' 're1re2' 'box1' '0.8306683066830668']]
'''
...
plt.ylabel(r'$n_s$')
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n1_ns/n1_ns_split.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n1_ns/n1_ns_contorno.png')
plt.close()
...
axs[2].legend()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/scatter_n_ratio_n2_geral.png')
plt.close()
...
axs[2].set_xlabel(label_x)
axs[2].legend()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/nxn2_sub_grupos.png')
plt.close()
...
axs.set_xlabel(label_x)
axs.legend()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/nxn2_elipticas.png')
plt.close()
...
axs.set_xlabel(label_x)
axs.legend()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/nxn2_true_cds.png')
plt.close()
...
axs.set_ylabel(label_y)
axs.set_xlabel(label_x)
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/nxn2_extra_light.png')
plt.close()

################################################

#MAPA DE COR - RAZÃO BT - SEM CORREÇÃO
os.makedirs(f'{sample}_stats_observation/plano_n_ratio_n2/bt',exist_ok=True)
fig1=plt.figure(figsize=(9,7),constrained_layout=True)
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt/nxn2_color_bt.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt/nxn2_color_bt_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt/nxn2_color_bt_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt/nxn2_color_bt_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt/nxn2_color_bt_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/bt/n_n2_bt_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt/nxn2_color_bt_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt/nxn2_color_bt_3d_only2c.png')
plt.close()
fig = plt.figure(figsize=(8, 7))
ax = fig.add_subplot(111, projection='3d')
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/bt/n_n2_bt_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt/nxn2_color_bt_3d_projections_only2c.png')
plt.close()

##################################
#MAPA DE COR - RAZÃO BT - CORRIGIDO
os.makedirs(f'{sample}_stats_observation/plano_n_ratio_n2/bt_corr',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
plt.suptitle('BT CORRIGIDO')
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt_corr/nxn2_color_bt_corr.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt_corr/nxn2_color_bt_true_cd_corr.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt_corr/nxn2_color_bt_elip_corr.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt_corr/nxn2_color_bt_e_el_corr.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt_corr/nxn2_color_bt_corr_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/bt_corr/n_n2_bt_corr_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt_corr/nxn2_color_bt_corr_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt_corr/nxn2_color_bt_corr_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/bt_corr/n_n2_bt_corr_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/bt_corr/nxn2_color_bt_corr_3d_projections_only2c.png')
plt.close()
###################################
#MAPA DE RAZÃO DE RFF
os.makedirs(f'{sample}_stats_observation/plano_n_ratio_n2/rff_ratio',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/rff_ratio/nxn2_color_rff_ratio.png')
plt.close()
...
plt.scatter(n_ratio_12[elip_lim],np.log10(n2)[elip_lim],c=rff_ratio_e,edgecolors='black',label='E',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'$RFF_{S+S}^2/RFF_{S}^2$', rotation=90)
plt.clim(0.,1.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/rff_ratio/nxn2_color_rff_ratio_elip.png')
plt.close()

#MAPA DE RAZÃO DE RFF - TRUE cDS

fig1=plt.figure(figsize=(9,7))
plt.scatter(n_ratio_12[lim_cd_big],np.log10(n2)[lim_cd_big],c=rff_ratio_cd_big,edgecolors='black',label='True cD',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'$RFF_{S+S}^2/RFF_{S}^2$', rotation=90)
plt.clim(0.,1.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/rff_ratio/nxn2_color_rff_ratio_true_cd.png')
plt.close()

#MAPA DE RAZÃO DE RFF - TRUE cDS

fig1=plt.figure(figsize=(9,7))
plt.scatter(n_ratio_12[lim_cd_small],np.log10(n2)[lim_cd_small],c=rff_ratio_cd_small,edgecolors='black',label='E(EL)',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'$RFF_{S+S}^2/RFF_{S}^2$', rotation=90)
plt.clim(0.,1.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/rff_ratio/nxn2_color_rff_ratio_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/rff_ratio/nxn2_color_rff_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/rff_ratio/n_n2_rff_ratio_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/rff_ratio/nxn2_color_rff_ratio_3d_projections.png')
plt.close()

#################################
#MAPA DE COR RAZÃO DE CHI2
os.makedirs(f'{sample}_stats_observation/plano_n_ratio_n2/chi2_ratio',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.xlabel(label_x)
plt.ylabel(label_y)
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/chi2_ratio/nxn2_color_chi2_ratio.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/chi2_ratio/nxn2_color_chi2_ratio_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/chi2_ratio/nxn2_color_chi2_ratio_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/chi2_ratio/nxn2_color_chi2_ratio_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/chi2_ratio/nxn2_color_chi2_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/chi2_ratio/n_n2_chi2_ratio_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/chi2_ratio/nxn2_color_chi2_ratio_3d_projections.png')
plt.close()

#################################
#MAPA DE COR DELTA BIC
os.makedirs(f'{sample}_stats_observation/plano_n_ratio_n2/delta_bic',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/delta_bic/nxn2_color_delta_bic.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/delta_bic/nxn2_color_delta_bic_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/delta_bic/nxn2_color_delta_bic_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/delta_bic/nxn2_color_delta_bic_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/delta_bic/nxn2_color_delta_bic_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/delta_bic/n_n2_delta_bic_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/delta_bic/nxn2_color_delta_bic_3d_projections.png')
plt.close()

###############################
#MAPA DE COR RAZÃO AXIAL
os.makedirs(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio/nxn2_color_q.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio/nxn2_color_q_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio/nxn2_color_q_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio/nxn2_color_q_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio/nxn2_color_ax_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio/n_n2_ax_ratio_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio/nxn2_color_ax_ratio_3d_projections.png')
plt.close()

###############################
#MAPA DE COR RAZÃO - Q1/Q2
os.makedirs(f'{sample}_stats_observation/plano_n_ratio_n2/q1q2',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/q1q2/nxn2_color_q_ratio.png')
plt.close()
...
plt.scatter(n_ratio_12[elip_lim],np.log10(n2)[elip_lim],c=axrat_ratio_e,edgecolors='black',label='E',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'$q_1/q_2$', rotation=90)
plt.clim(0.7,2.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/q1q2/nxn2_color_q_ratio_elip.png')
plt.close()
...
fig1=plt.figure(figsize=(9,7))
plt.scatter(n_ratio_12[lim_cd_big],np.log10(n2)[lim_cd_big],c=axrat_ratio_cd_big,edgecolors='black',label='True cD',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'$q$', rotation=90)
plt.clim(0.7,2.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/q1q2/nxn2_color_q_ratio_true_cd.png')
plt.close()

#MAPA DE COR RAZÃO Q1/Q2 - EXTRA LIGHT

fig1=plt.figure(figsize=(9,7))
plt.scatter(n_ratio_12[lim_cd_small],np.log10(n2)[lim_cd_small],c=axrat_ratio_cd_small,edgecolors='black',label='True cD',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'$q_1/q_2$', rotation=90)
plt.clim(0.7,2.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/q1q2/nxn2_color_q_ratio_e_el.png')
plt.close()
...
fig = plt.figure(figsize=(8, 7))
ax = fig.add_subplot(1,1,1,projection='3d')
ax.scatter(n_ratio_12[elip_lim],np.log10(n2)[elip_lim],axrat_ratio_e,alpha=0.4,c='green',edgecolor='black',label='E')
ax.scatter(n_ratio_12[lim_cd_small],np.log10(n2)[lim_cd_small],axrat_ratio_cd_small,alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
ax.scatter(n_ratio_12[lim_cd_big],np.log10(n2)[lim_cd_big],axrat_ratio_cd_big,alpha=0.6,c='red',edgecolor='black',label='True cD')
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/q1q2/nxn2_color_q_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/q1q2/n_n2_q_ratio_3d_spin.gif', dpi=100):
...
axs[0].legend()
#ZX
axs[1].scatter(axrat_ratio_e,n_ratio_12[elip_lim],alpha=0.4,c='green',edgecolor='black',label='E')
axs[1].scatter(axrat_ratio_cd_small,n_ratio_12[lim_cd_small],alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[1].scatter(axrat_ratio_cd_big,n_ratio_12[lim_cd_big],alpha=0.6,c='red',edgecolor='black',label='True cD')
axs[1].set_xlim([0.1,2.8])
axs[1].set_xlabel(r'$q_1/q_2$')
axs[1].set_ylabel(label_x)
axs[1].legend()
#ZY
axs[2].scatter(axrat_ratio_e,np.log10(n2)[elip_lim],alpha=0.4,c='green',edgecolor='black',label='E')
axs[2].scatter(axrat_ratio_cd_small,np.log10(n2)[lim_cd_small],alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[2].scatter(axrat_ratio_cd_big,np.log10(n2)[lim_cd_big],alpha=0.6,c='red',edgecolor='black',label='True cD')
axs[2].set_xlim([0.1,2.8])
axs[2].set_xlabel(r'$q_1/q_2$')
axs[2].set_ylabel(label_y)
axs[2].legend()
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/q1q2/nxn2_color_q_ratio_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/q1q2/nxn2_color_q_ratio_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/q1q2/n_n2_q_ratio_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/q1q2/nxn2_color_q_ratio_3d_projections_only2c.png')
plt.close()

###############################
#MAPA DE COR BOXINESS
os.makedirs(f'{sample}_stats_observation/plano_n_ratio_n2/box',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/box/nxn2_color_box_s.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/box/nxn2_color_box_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/box/nxn2_color_box_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/box/nxn2_color_box_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/box/nxn2_color_box_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/box/n_n2_box_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/box/nxn2_color_box_3d_projections.png')
plt.close()

################################
#MAPA DE COR RAZÃO - n1/n2
os.makedirs(f'{sample}_stats_observation/plano_n_ratio_n2/re1re2',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/re1re2/nxn2_color_re_ratio.png')
plt.close()
...
plt.scatter(n_ratio_12[elip_lim],np.log10(n2)[elip_lim],c=np.log10(re_ratio_e),edgecolors='black',label='E',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'${R_e}_{1}/{R_e}_{2}$', rotation=90)
plt.clim(-2,2.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/re1re2/nxn2_color_re_ratio_elip.png')
plt.close()

#MAPA DE COR RAZÃO n1/n2 - TRUE cDS

fig1=plt.figure(figsize=(9,7))
plt.scatter(n_ratio_12[lim_cd_big],np.log10(n2)[lim_cd_big],c=np.log10(re_ratio_cd_big),edgecolors='black',label='True cD',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'${R_e}_{1}/{R_e}_{2}$', rotation=90)
plt.clim(-2,2.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/re1re2/nxn2_color_re_ratio_true_cd.png')
plt.close()

#MAPA DE COR RAZÃO n1/n2 - EXTRA LIGHT

fig1=plt.figure(figsize=(9,7))
plt.scatter(n_ratio_12[lim_cd_small],np.log10(n2)[lim_cd_small],c=np.log10(re_ratio_cd_small),edgecolors='black',label='True cD',cmap=cmap)
cbar=plt.colorbar()
cbar.set_label(r'${R_e}_{1}/{R_e}_{2}$', rotation=90)
plt.clim(-2,2.)
plt.legend()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/re1re2/nxn2_color_re_ratio_e_el.png')
plt.close()
...
ax.scatter(n_ratio_12[elip_lim],np.log10(n2)[elip_lim],np.log10(re_ratio_e),alpha=0.4,c='green',edgecolor='black',label='E')
ax.scatter(n_ratio_12[lim_cd_small],np.log10(n2)[lim_cd_small],np.log10(re_ratio_cd_small),alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
ax.scatter(n_ratio_12[lim_cd_big],np.log10(n2)[lim_cd_big],np.log10(re_ratio_cd_big),alpha=0.6,c='red',edgecolor='black',label='True cD')
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/re1re2/nxn2_color_re_ratio_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/re1re2/n_n2_re_ratio_3d_spin.gif', dpi=100):
...
axs[0].legend()
#ZX
axs[1].scatter(np.log10(re_ratio_e),n_ratio_12[elip_lim],alpha=0.4,c='green',edgecolor='black',label='E')
axs[1].scatter(np.log10(re_ratio_cd_small),n_ratio_12[lim_cd_small],alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[1].scatter(np.log10(re_ratio_cd_big),n_ratio_12[lim_cd_big],alpha=0.6,c='red',edgecolor='black',label='True cD')
axs[1].set_xlim([-2,2.])
axs[1].set_xlabel(r'${R_e}_1/{R_e}_2$')
axs[1].set_ylabel(label_x)
axs[1].legend()
#ZY
axs[2].scatter(np.log10(re_ratio_e),np.log10(n2)[elip_lim],alpha=0.4,c='green',edgecolor='black',label='E')
axs[2].scatter(np.log10(re_ratio_cd_small),np.log10(n2)[lim_cd_small],alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
axs[2].scatter(np.log10(re_ratio_cd_big),np.log10(n2)[lim_cd_big],alpha=0.6,c='red',edgecolor='black',label='True cD')
axs[2].set_xlim([-2,2])
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/re1re2/nxn2_color_re_ratio_3d_projections.png')
plt.close()
...
ax.scatter(n_ratio_12[lim_cd_small],np.log10(n2)[lim_cd_small],re_ratio_cd_small,alpha=0.5,c='blue',edgecolor='black',label='E(EL)')
ax.scatter(n_ratio_12[lim_cd_big],np.log10(n2)[lim_cd_big],re_ratio_cd_big,alpha=0.6,c='red',edgecolor='black',label='True cD')
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/re1re2/nxn2_color_re_ratio_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/re1re2/n_n2_re_ratio_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/re1re2/nxn2_color_re_ratio_3d_projections_only2c.png')
plt.close()

################################
#MAPA DE COR - INDICE DE SÉRSIC
os.makedirs(f'{sample}_stats_observation/plano_n_ratio_n2/n_sersic',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/n_sersic/nxn2_color_n.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/n_sersic/nxn2_color_n_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/n_sersic/nxn2_color_n_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/n_sersic/nxn2_color_n_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/n_sersic/nxn2_color_n_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/n1n2/n_n2_n_ratio_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/n_sersic/nxn2_color_n_3d_projections.png')
plt.close()

##########################
#MAPA DE COR - INDICE DE SÉRSIC 1 - SS
os.makedirs(f'{sample}_stats_observation/plano_n_ratio_n2/n1',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/n1/nxn2_color_n1.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/n1/nxn2_color_n1_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/n1/nxn2_color_n1_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/n1/nxn2_color_n1_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/n1/nxn2_color_n1_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/n1/n_n2_n1_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/n1/nxn2_color_n1_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/n1/nxn2_color_n1_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/n1/n_n2_n1_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/n1/nxn2_color_n1_3d_projections_only2c.png')
plt.close()

##########################
#MAPA DE COR - RAZÃO AXIAL 1 - SS
os.makedirs(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_1',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_1/nxn2_color_q1.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_1/nxn2_color_q1_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_1/nxn2_color_q1_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_1/nxn2_color_q1_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_1/nxn2_color_q1_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_1/n_n2_q1_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_1/nxn2_color_q1_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_1/nxn2_color_q1_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_1/n_n2_q1_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_1/nxn2_color_q1_3d_projections_only2c.png')
plt.close()

##########################
#MAPA DE COR - RAZÃO AXIAL 2 - SS
os.makedirs(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_2',exist_ok=True)
fig1=plt.figure(figsize=(9,7))
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_2/nxn2_color_q2.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_2/nxn2_color_q2_elip.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_2/nxn2_color_q2_true_cd.png')
plt.close()
...
plt.ylabel(label_y)
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_2/nxn2_color_q2_e_el.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_2/nxn2_color_q2_3d.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_2/n_n2_q2_3d_spin.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_2/nxn2_color_q2_3d_projections.png')
plt.close()
...
plt.tight_layout()
# plt.show()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_2/nxn2_color_q2_3d_only2c.png')
plt.close()
...
with writer.saving(fig, f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_2/n_n2_q2_3d_spin_only2c.gif', dpi=100):
...
plt.tight_layout()
plt.savefig(f'{sample}_stats_observation/plano_n_ratio_n2/ax_ratio_2/nxn2_color_q2_3d_projections_only2c.png')
plt.close()
# #'''#########################################
...
# ax.scatter(np.log10(re_ratio_12)[lim_cd_small],n2[lim_cd_small],np.log10(bt_vec_corr[lim_cd_small]),edgecolors='black',color='blue')
ax.scatter(np.log10(n_ratio_12)[lim_cd_small],n2[lim_cd_small],(delta_bic_obs[lim_cd_small]),c='blue',edgecolor='black',label='E(EL)')
ax.scatter(np.log10(n_ratio_12)[lim_cd_big],n2[lim_cd_big],(delta_bic_obs[lim_cd_big]),c='red',edgecolor='black',label='True cD')
# ax.scatter(re_1_kpc[lim_cd_big],mue_1[lim_cd_big],(bt_vec_corr[lim_cd_big]),edgecolors='black',color='red')
# ax.scatter(re_1_kpc[lim_cd_small],mue_1[lim_cd_small],(bt_vec_corr[lim_cd_small]),edgecolors='black',color='blue')
# ax.scatter(np.log10(re_ratio_12)[lim_cd_small],n2[lim_cd_small],np.log10(bt_vec_corr[lim_cd_small]),edgecolors='black',color='blue')
...
ax_righty.tick_params(labelleft=False)
# plt.show()
plt.savefig(f'{sample}_stats_observation/rel_kormendy_sample_sersic.png')
plt.close()
...
ax_righty.legend(fontsize='small')
ax_righty.tick_params(labelleft=False)
plt.savefig(f'{sample}_stats_observation/rel_kormendy_sample_sersic_small.png')
plt.close()
...
ax_righty.tick_params(labelleft=False)
# plt.show()
plt.savefig(f'{sample}_stats_observation/rel_kormendy_sample_sersic_big.png')
plt.close()
###
...
ax_righty.tick_params(labelleft=False)
# plt.show()
plt.savefig(f'{sample}_stats_observation/rel_kormendy_sample.png')
plt.close()
...
ax_righty.tick_params(labelleft=False)
# plt.show()
plt.savefig(f'{sample}_stats_observation/rel_kormendy_small.png')
plt.close()
####
fig = plt.figure(figsize=(10, 10))
gs = gridspec.GridSpec(2, 2, width_ratios=[5,1], height_ratios=[1,5],hspace=0.05, wspace=0.05)
...
ax_righty.tick_params(labelleft=False)
# plt.show()
plt.savefig(f'{sample}_stats_observation/rel_kormendy_big.png')
plt.close()

r'''
###
#OBTENÇÃO DO PONTO DE CORTE DAS ELIPTICAS NUCLEARES EM RELAÇÃO AO RESTANTE

x,y=re_intern, mue_intern
y_err=None
...
ax1.scatter(x, y, alpha=0.7, label=label_bojo)
...
plt.tight_layout()
plt.show()
# plt.savefig(f'{sample}_stats_observation/two_line_fit_re_intern.png')
plt.close()
'''
#########################################################1####
...
# plt.show()
plt.savefig(f'{sample}_stats_observation/p_value_delta_bic/p_value_delta_bic_lines.png')
plt.close(fig)
...
cbar.set_label(r'$n$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/p_value_delta_bic/p_value_delta_bic_n_simples.png')
plt.close(fig)
...
cbar.set_label(r'$q$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/p_value_delta_bic/p_value_delta_bic_axrat.png')
plt.close()
...
# plt.show()
plt.savefig(f'{sample}_stats_observation/p_value_delta_bic/p_value_delta_bic_box.png')
plt.close(fig)
...
cbar.set_label(r'$B/T$')
# plt.show()
plt.savefig(f'{sample}_stats_observation/p_value_delta_bic/p_value_delta_bic_bt.png')
plt.close(fig)
...
# plt.show()
plt.savefig(f'{sample}_stats_observation/p_value_delta_bic/p_value_delta_bic_re_ratio.png')
plt.close(fig)
...
# plt.show()
plt.savefig(f'{sample}_stats_observation/p_value_delta_bic/p_value_delta_bic_n_ratio.png')
plt.close(fig)

#RAZÃO DE RAIOS (TESTE)
# fig,axs=plt.subplots(1,3,sharey=True,tight_layout=True,figsize=(15,5))
# axs[0].scatter(delta_bic_obs[lim_cd_small],p_value[lim_cd_small],c=re_ratio_12[lim_cd_small],edgecolor='black',vmax=1.5,alpha=0.3,cmap=cmap_r)
# axs[0].scatter(delta_bic_obs[lim_cd_big],p_value[lim_cd_big],c=re_ratio_12[lim_cd_big],edgecolor='black',vmax=1.5,alpha=0.3,cmap=cmap_r)
# axs[0].scatter(delta_bic_obs[elip_lim],p_value[elip_lim],c=re_ratio_12[elip_lim],edgecolor='black',vmax=1.5,alpha=0.9,label='E',cmap=cmap_r)
# axs[0].axhline(0.32)
# line_cmap=plt.colormaps['jet']
# line_colors=line_cmap(np.linspace(0,1,len(vec_cuts)))
...
# plt.show()
plt.savefig(f'{sample}_stats_observation/p_value_delta_bic/p_value_delta_bic_q_ratio.png')
plt.close(fig)
...
# plt.show()
# plt.savefig(f'{sample}_stats_observation/p_value_delta_bic_lines.png')
plt.close(fig)

fig,axs=plt.subplots(1,1,tight_layout=True,figsize=(7,7))
axs.scatter(np.log10(n2),n_ratio_12,color='white',edgecolor='black',alpha=0.2,label='WHL')
axs.scatter(n2[high_lim_cd],n_ratio_12[high_lim_cd],color='red',edgecolor='black',alpha=0.4,label='n1>2.5')
axs.scatter(n2[low_lim_cd],n_ratio_12[low_lim_cd],color='blue',edgecolor='black',alpha=0.6,label='n1<2.5')
axs.scatter(n2[higher_lim_cd & (re_1_kpc<0.5)],n_ratio_12[higher_lim_cd & (re_1_kpc<0.5)],color='orange',edgecolor='black',alpha=1,label='n1>9.5')
axs.legend()
axs.set_xlabel(r'$n_2$')#$Re_1/Re_2$')
axs.set_ylabel(r'$n_1/n_2$')
axs.set_xlim(0.4,15)
# plt.show()
# plt.savefig(f'{sample}_stats_observation/p_value_delta_bic_lines.png')
...
for item in num_n1:
	...
	# axs.set_ylabel(ylabel)
	# axs.legend()
	# plt.savefig(f'{sample}_stats_observation/analise_sersic_cds/{param[0]}_{item-1}.png')
	# plt.close()
	...
	axs.legend()
	# plt.show()
	# plt.savefig(f'{sample}_stats_observation/analise_comp_cds/intern/{param[0]}_intern_{item-1}.png')
	plt.close()
	...
	axs.set_ylabel(ylabel)
	axs.legend()
	# plt.savefig(f'{sample}_stats_observation/analise_comp_cds/extern/{param[0]}_extern_{item-1}.png')
	plt.close()
	...
	# axs[1].legend()
	# # plt.show()
	# # plt.savefig(f'{sample}_stats_observation/analise_comp_cds/both/{param[0]}_{item-1}.png')
	# plt.close()

#GIF BUILDER

writer=PillowWriter(fps=0.5)
gif_name='WHL_stats_observation/gif_n1_extern_comp_07.gif'
...
axs.legend()
# plt.show()
plt.savefig(f'{sample}_stats_observation/p_value/rel_kormendy_cd_e_low.png')
plt.close()
####
##OBJETOS COM N1>2.5
...
axs.legend()
plt.show()
# plt.savefig(f'{sample}_stats_observation/p_value/rel_kormendy_cd_e_high.png')
plt.close()
####
...
axs.legend()
# plt.show()
# plt.savefig(f'{sample}_stats_observation/p_value/kormendy_rel_comp_low.png')
plt.close()
...
# plt.show()
# plt.savefig(f'{sample}_stats_observation/p_value/geral/morf_class_bojo_box_{str(file_names[n-1])}.png')
plt.close()
...
cbar.set_label(r'$B/T (12)$')
# plt.show()
plt.savefig('L07_stats_observation/scatter_re_ratio_n2_bt_12_geral.png')
plt.close()
...
# plt.legend()
# plt.show()
# # plt.savefig(f'{sample}_stats_observation/best_ncomp.png')
# plt.close(fig)
...
# plt.savefig('L07_stats_observation/delta_bic_sim_full.png')
plt.show()
plt.close()
...
# plt.savefig('L07_stats_observation/delta_bic_sim_close.png')
# plt.close()
...
# plt.savefig('L07_stats_observation/delta_bic_sim_mid_close.png')
# plt.close()
...
r'''
for n in num:
	# os.makedirs(f'{sample}_stats_observation/p_value/kormendy_rel',exist_ok=True)
	z_sub_sample_only=idx_faixas==n
	# z_cuts.append(np.average(redshift[z_sub_sample_only]))
	lim1=rff_ratio[z_sub_sample_only]>0.9
	delta_bic_obs_sub_sample=delta_bic_obs[z_sub_sample_only]
	...
	# np.savetxt(f'{sample}_stats_observation/data_indiv_n1_{n-1}.dat',data_n1,fmt='%s',newline='\n')
	#######
	...
	# plt.show()
	# plt.savefig(f'{sample}_stats_observation/p_value/{str(file_names[n-1])}/rel_kormendy_s_e_cd_{str(file_names[n-1])}.png')
	# plt.savefig(f'{sample}_stats_observation/p_value/rel_kormendy/rel_kormendy_s_e_cd_{str(file_names[n-1])}.png')
	plt.close()
	...
	# axs.legend()
	# # plt.show()
	# # plt.savefig(f'{sample}_stats_observation/p_value/morf_class_allin_{str(file_names[n-1])}.png')
	# plt.close()
...
axs[1].set_ylabel(r'$Delta BIC$')
axs[1].set_ylim(2000,-8000)
plt.savefig('L07_stats_observation/delta_bic_z_t1t2_morf.png')
plt.close()
'''
r'''
...
# plt.show()
plt.savefig('L07_stats_observation/rffxeta_morf_types.png')
plt.close()
...
plt.setp(ax,xlabel=(r'$\log\,RFF$'))
# plt.show()
plt.savefig('L07_stats_observation/rffxeta_morf_types_t1.png')
plt.close()
...
plt.setp(ax,xlabel=(r'$\log\,RFF$'))
# plt.show()
# plt.savefig('L07_stats_observation/rffxeta_morf_types_t2.png')
plt.close()
...
# 		g=3
# 		caminho=f'{sample}_stats_observation/redshift_gmm/{pasta}_bins/{q+1}/n_{g}_comp_stats.dat'
...
# 		plt.legend()
# 		# plt.show()
# 		plt.savefig(f'{sample}_stats_observation/redshift_gmm/graficos_yang/best_ncomp_{pasta-1}_{q}.png')
# 		plt.close(fig)
...
# plt.tight_layout()
# # plt.show()
# plt.savefig(f'{sample}_stats_observation/redshift_gmm/graficos_yang/tabela_{sample}_confusion.png')
	...
	# 	plt.show()
	# 	# plt.savefig(f'{sample}_stats_observation/redshift_gmm/graficos_yang/{pasta-1}_bins_{g_idx+2}_comp.png')
	# 	plt.close()
'''
...
# plt.savefig('L07_stats_observation/confusion_matrix_morf.png')
'''
...
r'''
# 	os.makedirs(f'WHL_stats_observation/redshift_gmm/5_bins/{i}',exist_ok=True)
# 	for n in range(1000):
...
# for i in range(1,7):
# 	os.makedirs(f'WHL_stats_observation/redshift_gmm/7_bins/{i}',exist_ok=True)
...
# idx_faixas=np.digitize(redshift,faixas)
# for i in range(1,10):
# 	os.makedirs(f'WHL_stats_observation/redshift_gmm/10_bins/{i}',exist_ok=True)
# 	bic_vec=delta_bic[idx_faixas==i]
# 	for n in range(1000):
# 		print('bin',i,'BOOTSTRAP',n+1)
# 		random_delta_bic=np.random.choice(bic_vec,len(bic_vec))
