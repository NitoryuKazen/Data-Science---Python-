...
# obtencao do numero da bcg e formatacao do tamanho da imagem 
	...
	#SUPY
		...
		header.append(card='XC',useblanks=True, bottom=False, end=False)
		header['XC'] = float(xc)
		
		header.append(card='YC',useblanks=True, bottom=False, end=False)
		header['YC'] = float(yc)
##########################################################
...
# obtencao do numero da bcg do small.cat
	...
	######################################
		...
		if os.path.isfile(cluster+'/bcg_g_mask.fits'):
			mask = pyfits.getdata(cluster+'/bcg_g_mask.fits')
			mask_b = pyfits.getdata(cluster+'/bcg_g_mask_b.fits')
...
#########################################################################################
		...
		maskr=pyfits.getdata(cluster+'/bcg_r_mask.fits')
		maskr_b=pyfits.getdata(cluster+'/bcg_r_mask_b.fits')
		header_maskr=pyfits.getheader(cluster+'/bcg_r_mask.fits')
		...
		if area_g <= area_r:
			...
			difxc= mask.shape[1]/2 - maskr.shape[1]/2
			difyc= mask.shape[0]/2 - maskr.shape[0]/2 
			...
			mask_comb=copy.deepcopy(mask)
			mask_comb_b=copy.deepcopy(mask_b)
			for i in range(0,mask.shape[0]):
				for j in range(0,mask.shape[1]):
					if nmvec[i,j] == 1 or mask[i,j] == 1:
						mask_comb[i,j] = 1
						mask_comb_b[i,j] = 1
			...
			pyfits.writeto(cluster+'/bcg_gr_mask.fits',mask_comb,header=header,clobber=True)
			pyfits.writeto(cluster+'/bcg_gr_b_mask.fits',mask_comb_b,header=header,clobber=True)

		#g>r
		
		if area_g >= area_r:

			difxc= maskr.shape[1]/2 - mask.shape[1]/2
			difyc= maskr.shape[0]/2 - mask.shape[0]/2 
			...
			mask_comb=copy.deepcopy(maskr)
			mask_comb_b=copy.deepcopy(maskr_b)
			for i in range(0,maskr.shape[0]):
				for j in range(0,maskr.shape[1]):
					if nmvec[i,j] == 1 or maskr[i,j] == 1:
						mask_comb[i,j] = 1
						mask_comb_b[i,j] = 1
...
'''
blank_img=copy.deepcopy(maskr[0:mask.shape[0],0:mask.shape[1]])


for i in range(0,mask.shape[0]):
	for j in range(0,mask.shape[1]):
			blank_img[i,j]=0	
pyfits.writeto(cluster+'/test.fits',blank_img,header=header_maskr,clobber=True)

fig, ax = plt.subplots(figsize=(6, 6))
ax.imshow(blank_img,vmin=0,vmax=1,origin='lower')
plt.show()
