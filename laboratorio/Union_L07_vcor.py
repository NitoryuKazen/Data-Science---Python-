...
#PROGRAMA PARTE 1 
	...
	par = open('pargal_L07.dat','a')
	info_err=open('aux_pargal_L07.dat','a')
...
# ASSIMETRIA GRANDE
	...
	par.write('%s \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f\n'%(cluster,chi2,re,mag,n,axis,pa,float(rff),float(A0),float(A1),delta,gama))
	par.close()
	info_err.write('%s \t %f \t %f \t %f \t %f \t %f \n'%(cluster,re_err,mag_err,n_err,axis_err,pa_err))
...
ok=[]
with open('pargal_L07.dat','r') as inp2:
