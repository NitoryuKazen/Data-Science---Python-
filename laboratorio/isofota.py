...
###################################################################3
	...
	if [flagmask,flagdelta] == [0,0]:
		...
		if cluster in ok:# or cluster == '1417':
			...
			call('mkdir L07_maskcor/'+cluster,shell=True)	
...
##########################################################################3
		#AJUSTE DAS ISOFOTAS
			...
			if os.path.isfile('L07_maskcor/'+cluster+'/iso_table.dat'):
				temp=[[] for i in range(9)]
				iso_table=open('L07_maskcor/'+cluster+'/iso_table.dat','r')
				...
				x0,y0,sma,pa,eps,intens,ellip_err,pa_err,int_err=vec				
	...
	#########################################################################
			#VERIFICACAO DE CONVERGENCIA
				...
				plt.ylabel('Y (pix)')
				plt.tight_layout()
				plt.savefig('L07_maskcor/'+cluster+'/'+cluster+'_iso_free.png')
				plt.close(fig)
	...
	###########################################################################################
			#VALORES DE INTERESSE
				...
				inptrue=open('L07_maskcor/'+cluster+'/iso_table.dat','w')
...
###################################################################################
		# PLOT DE PARAMETROS GERAIS RAIOX(MAGNITUDE,ELIPTICIDADE,PA)
			...
			plt.ylabel('PA (deg)')
			plt.tight_layout()
			plt.savefig('L07_maskcor/'+cluster+'/'+cluster+'_iso_info.png')	
			plt.close()	
######################################################################################################
			...
			plt.legend()
			plt.tight_layout()
			plt.savefig('L07_maskcor/'+cluster+'/'+cluster+'_s_se_models.png')	
			plt.close()	
###############################################################################################
###############################################################################################
###############################################################################################
########ELIPSES FIXAS COM OS VALORES OBTIDOS PARA A ULTIMA ELIPSE OBTIDA PELO GALFIT
			if os.path.isfile('L07_maskcor/'+cluster+'/galfix_table.dat'):
				smaintens=[[] for i in range(3)]
				galfix_table=open('L07_maskcor/'+cluster+'/galfix_table.dat','r')
				for item in galfix_table.readlines():
					for i in range(3):
						smaintens[i].append(float(item.split()[i]))
				...
				plt.ylabel('Y (pix)')
				plt.tight_layout()
				plt.savefig('L07_maskcor/'+cluster+'/'+cluster+'_galfix_ellipses.png')	
				plt.close(fig)
				gtable=open('L07_maskcor/'+cluster+'/galfix_table.dat','w')
				for i in range(len(smaintens[0])):
					gtable.write('%f \t %f \t %f\n'%(smaintens[0][i],smaintens[1][i],smaintens[2][i]))
				gtable.close()
####################################################################################################
		# AJUSTES DAS LEIS DE DE VAUCOULERS, SERSIC E SERSIC + EXPONENCIAL RESPECTIVAMENTE
...
###########################################################################################################
		# SERSIC & SERSIC + EXP
			...
			plt.legend()
			plt.tight_layout()
			plt.savefig('L07_maskcor/'+cluster+'/'+cluster+'_s_se_models_galfix.png')	
			plt.close()
##############################################################################################
##############################################################################################
##############################################################################################
########ELIPSES FIXAS COM OS VALORES OBTIDOS PELO PHOTUTILS PARA A ULTIMA ELIPSE
			...
			if os.path.isfile('L07_maskcor/'+cluster+'/photfix_table.dat'):
				smaintens=[[] for i in range(3)]
				photfix_table=open('L07_maskcor/'+cluster+'/photfix_table.dat','r')
				for item in photfix_table.readlines():
					for i in range(3):
						smaintens[i].append(float(item.split()[i]))
			else:
				fig, ax = plt.subplots(figsize=(6, 6))
				...
				plt.ylabel('Y (pix)')
				plt.tight_layout()
				plt.savefig('L07_maskcor/'+cluster+'/photofix_ellipses.png')	
				plt.close(fig)

				ptable=open('L07_maskcor/'+cluster+'/photfix_table.dat','w')
				for i in range(len(smaintens[0])):
					ptable.write('%f \t %f \t %f\n'%(smaintens[0][i],smaintens[1][i],smaintens[2][i]))
				ptable.close()
...
##############################################################################################
		# SERSIC & SERSIC + EXP
			...
			plt.legend()
			plt.tight_layout()
			plt.savefig('L07_maskcor/'+cluster+'/'+cluster+'_s_se_models_photofix.png')
			plt.close()
			
			output.write('%s %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n'%(cluster,sky,chisq_galfit,skyvalue,sigmasky,ellipgrad,pagrad,extval_ellip/ell0,extval_pa-pa0,chirat_1,chirat_2,chirat_3,chisq_s_1,chisq_s_2,chisq_s_3,spopt[0],spopt[1],spopt[2],sepopt[0],sepopt[1],sepopt[2],sepopt[3],sepopt[4],	vpopt[0],vpopt[1],fixspopt[0],fixspopt[1],fixspopt[2],fixsepopt[0],fixsepopt[1],fixsepopt[2],fixsepopt[3],fixsepopt[4],fixvpopt[0],fixvpopt[1],pixspopt[0],pixspopt[1],pixspopt[2],pixsepopt[0],pixsepopt[1],pixsepopt[2],pixsepopt[3],pixsepopt[4],pixvpopt[0],pixvpopt[1]))
##############################################################################################
